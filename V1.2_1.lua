...
function Library:CreateWindow(titleText)
    ...
    function Window:CreateTab(name)
        ...
        function Tab:Section(data)
            return self:CreateButton("--- " .. (data.Title or "Section") .. " ---", nil)
        end
        ...
        function Tab:CreateDropdown(text, options, callback)
            ...
            local OptionsFrame = Create("Frame", {Size = UDim2.new(1, 0, 0, #options * 30), Position = UDim2.fromOffset(0, 35), BackgroundTransparency = 1, Parent = DropFrame}, {Create("UIListLayout", {Padding = UDim.new(0, 2)})})
            
            local opened = false
            DropBtn.MouseButton1Click:Connect(function()
                opened = not opened
                TweenService:Create(DropFrame, TweenInfo.new(0.3), {Size = opened and UDim2.new(0.96, 0, 0, 35 + (#options * 30)) or UDim2.new(0.96, 0, 0, 35)}):Play()
            end)
            ...
            function DropObj:Add(val)
                AddOption(val)
                OptionsFrame.Size = UDim2.new(1, 0, 0, #OptionsFrame:GetChildren() * 30)
            end
            return DropObj
        end
