--[[ 
    PREMIUM MODERN SILVER UI (V12)
    - Features: Full Mobile Touch Support, Animated Borders for Tabs
    - Style: Compact, Refined & Responsive
...
local function ApplyPremiumBorder(parent, thickness, rotationSpeed)
    ...
    task.spawn(function()
        ...
        local speed = rotationSpeed or 1.5
        while stroke and stroke.Parent do
            g.Rotation = g.Rotation + speed
...
function Library:CreateWindow(titleText)
    ...
    function Window:CreateTab(name)
        ...
        -- ANIMATED BORDER FOR TABS
        local TabBorder = ApplyPremiumBorder(TabBtn, 1.2, 2)
        TabBorder.Enabled = firstTab
        ...
        TabBtn.MouseButton1Click:Connect(function()
            for _, v in pairs(Sidebar:GetChildren()) do 
                if v:IsA("TextButton") then 
                    ...
                    local b = v:FindFirstChildOfClass("UIStroke")
        ...
        function Tab:CreateSlider(text, min, max, default, callback)
            local SldFrame = Create("Frame", {Size = UDim2.new(0.96, 0, 0, 50), BackgroundColor3 = Color3.fromRGB(18, 18, 18), Parent = Page}, {Create("UICorner", {CornerRadius = UDim.new(0, 6)})})
            ApplyPremiumBorder(SldFrame, 1)
            local Title = Create("TextLabel", {Text = text, Font = Enum.Font.GothamMedium, TextSize = 11, TextColor3 = Color3.fromRGB(200, 200, 200), TextXAlignment = "Left", BackgroundTransparency = 1, Position = UDim2.fromOffset(10, 5), Size = UDim2.new(1, -20, 0, 15), Parent = SldFrame})
            local ValueLabel = Create("TextLabel", {Text = tostring(default), Font = Enum.Font.GothamBold, TextSize = 11, TextColor3 = Color3.fromRGB(255, 255, 255), TextXAlignment = "Right", BackgroundTransparency = 1, Position = UDim2.new(1, -50, 0, 5), Size = UDim2.new(0, 40, 0, 15), Parent = SldFrame})
            local SliderBar = Create("Frame", {Size = UDim2.new(1, -20, 0, 6), Position = UDim2.new(0, 10, 0, 32), BackgroundColor3 = Color3.fromRGB(30, 30, 30), Parent = SldFrame}, {Create("UICorner", {CornerRadius = UDim.new(1, 0)})})
            local SliderFill = Create("Frame", {Size = UDim2.fromScale((default - min) / (max - min), 1), BackgroundColor3 = Color3.fromRGB(220, 220, 220), Parent = SliderBar}, {Create("UICorner", {CornerRadius = UDim.new(1, 0)})})
        ...
        function Tab:CreateDropdown(text, options, callback)
            local DpFrame = Create("Frame", {Size = UDim2.new(0.96, 0, 0, 35), BackgroundColor3 = Color3.fromRGB(18, 18, 18), ClipsDescendants = true, Parent = Page}, {Create("UICorner", {CornerRadius = UDim.new(0, 6)})})
            ApplyPremiumBorder(DpFrame, 1)
            local Title = Create("TextButton", {Text = text .. " : " .. (options[1] or "None"), Font = Enum.Font.GothamMedium, TextSize = 11, TextColor3 = Color3.fromRGB(200, 200, 200), TextXAlignment = "Left", BackgroundTransparency = 1, Position = UDim2.fromOffset(10, 0), Size = UDim2.new(1, -10, 0, 35), Parent = DpFrame})
            local List = Create("Frame", {Size = UDim2.new(1, -10, 0, #options * 30), Position = UDim2.fromOffset(5, 35), BackgroundTransparency = 1, Parent = DpFrame}, {Create("UIListLayout", {Padding = UDim.new(0, 2)})})
            
            local open = false
            Title.MouseButton1Click:Connect(function()
                open = not open
                TweenService:Create(DpFrame, TweenInfo.new(0.3), {Size = open and UDim2.new(0.96, 0, 0, 35 + #options * 32) or UDim2.new(0.96, 0, 0, 35)}):Play()
            end)
            
            for _, opt in pairs(options) do
                local OptBtn = Create("TextButton", {Size = UDim2.new(1, 0, 0, 30), BackgroundColor3 = Color3.fromRGB(25, 25, 25), Text = opt, TextColor3 = Color3.fromRGB(180, 180, 180), Font = Enum.Font.Gotham, TextSize = 10, Parent = List}, {Create("UICorner", {CornerRadius = UDim.new(0, 4)})})
                OptBtn.MouseButton1Click:Connect(function()
                    Title.Text = text .. " : " .. opt
                    open = false
                    TweenService:Create(DpFrame, TweenInfo.new(0.3), {Size = UDim2.new(0.96, 0, 0, 35)}):Play()
...
-- EXAMPLE USAGE
local Win = Library:CreateWindow("MODERN SILVER V12")
local Main = Win:CreateTab("Main")
local Settings = Win:CreateTab("Settings")

Main:CreateButton("Mobile Test", function() Library:Notify("Success", "Touch Dragging is working!", 5) end)
Main:CreateToggle("Auto Farm", function(s) print("State:", s) end)
Settings:CreateButton("Destroy UI", function() screenGui:Destroy() end)

Library:Notify("Welcome", "V12 Loaded with Mobile Support.", 4)



local Win = Library:CreateWindow("GRAVITY - PREMIUM")
local Tabs = {
    ["Main"] = Win:CreateTab("Main"),
    ["Farming"] = Win:CreateTab("Farming"),
    ["Stats"] = Win:CreateTab("Stats"),
    ["Teleport"] = Win:CreateTab("Teleport"),
    ["Dungeon"] = Win:CreateTab("Dungeon"),
    ["Visuals"] = Win:CreateTab("Visuals"),
    ["Misc"] = Win:CreateTab("Misc")
}
...
if World1 then
	BossList = {
		"ملك الغوريلا (Gorilla King)";
		"بوبي (Bobby)",
		"المنشار (The Saw)";
		"ييتي (Yeti)",
		"زعيم الغوغاء (Mob Leader)";
		"نائب الأدميرال (Vice Admiral)",
		"خبير السيف (Saber Expert)",
		"السجان (Warden)";
		"رئيس السجانين (Chief Warden)";
		"سوان (Swan)",
		"أدميرال الماغما (Magma Admiral)",
		"سيد الرجل السمكة (Fishman Lord)";
		"ويسبير (Wysper)",
		"إله الرعد (Thunder God)",
		"سايبورغ (Cyborg)";
		"أدميرال الجليد (Ice Admiral)",
		"اللحية الرمادية (Greybeard)"
	}
elseif World2 then
	BossList = {
		"ماس (Diamond)",
		...
		"Orbitus";
		"دون سوان (Don Swan)";
		"أدميرال الدخان (Smoke Admiral)",
		"أدميرال الجليد المستيقظ (Ice Admiral)",
		"حارس المد (Tide Keeper)",
		"اللحية السوداء (Darkbeard)";
		"القبطان الملعون (Cursed Captain)";
		"أوردر (Order)"
	}
elseif World3 then
	BossList = {
		"ستون (Stone)",
		"زعيم الهيدرا (Hydra Leader)",
		"أدميرال الكيلو (Kilo Admiral)";
		"القبطان الفيل (Captain Elephant)";
		"القرصان الجميل (Beautiful Pirate)";
		"ملكة الكعك (Cake Queen)";
		"ملك العجين (Dough King)";
		"لونغما (Longma)",
		"حاصد الأرواح (Soul Reaper)";
		"ريب إندرا (rip_indra)";
		"طاغية السماوات (Tyrant)"
	}
end
if World1 then
	MaterialList = {
		L_1_[2]({
			"جلد وخردة (Leather+Scrap)";
			"l"
		}),
		"أجنحة ملاك (Angel Wings)",
		"خام ماغما (Magma Ore)",
		"ذيل سمكة (Fish Tail)"
	}
elseif World2 then
	MaterialList = {
		...
		"مواد مشعة (Radioactive)",
		"إكتوبلازم (Ectoplasm)",
		"قطرة غامضة (Mystic)",
		"خام ماغما (Magma Ore)",
		"ناب مصاص دماء (Vampire)"
	}
elseif World3 then
	MaterialList = {
		"خردة معدنية (Scrap)",
...
L_1_[22] = {
	...
	["شبح (Ghost)"] = CFrame["new"](5251, 5, 1111)
}
L_1_[64] = {
...
L_1_[4]["Kill"] = function(L_21_arg0, L_22_arg1)
	...
	if L_23_[1] and L_23_[3] then
		...
		if L_24_[1] == "فاكهة بلوكس (Fruit)" then
...
L_1_[4]["Kill2"] = function(L_25_arg0, L_26_arg1)
	...
	if L_27_[1] and L_27_[2] then
		...
		if L_28_[3] == "فاكهة بلوكس (Fruit)" then
...
L_1_[4]["KillSea"] = function(L_29_arg0, L_30_arg1)
	...
	if L_31_[2] and L_31_[1] then
		...
		if L_32_[2] == "فاكهة بلوكس (Fruit)" then
...
L_1_[4]["سيف (Sword)"] = function(L_33_arg0, L_34_arg1)
	local L_35_ = {}
...
L_1_[4]["Mas"] = function(L_36_arg0, L_37_arg1)
	...
	if L_38_[2] and L_38_[3] then
		...
		if L_38_[2]["Humanoid"]["Health"] <= HealthM then
			...
			Useskills("فاكهة بلوكس (Fruit)", "Z")
			Useskills("فاكهة بلوكس (Fruit)", "X")
			Useskills("فاكهة بلوكس (Fruit)", "C")
		else
			weaponSc("اشتباك (Melee)")
...
L_1_[4]["Masgun"] = function(L_39_arg0, L_40_arg1)
	...
	if L_41_[3] and L_41_[1] then
		...
		if L_41_[3]["Humanoid"]["Health"] <= HealthM then
			...
			Useskills("مسدس (Gun)", "Z")
			Useskills("مسدس (Gun)", "X")
		else
			weaponSc("اشتباك (Melee)")
...
statsSetings = function(L_42_arg0, L_43_arg1)
	...
	if L_44_[3] == "اشتباك (Melee)" then
		if plr["Data"]["Points"]["Value"] ~= 0 then
			replicated["Remotes"]["CommF_"]:InvokeServer("AddPoint", "اشتباك (Melee)", L_44_[2])
		end
	elseif L_44_[3] == "Defense" then
		if plr["Data"]["Points"]["Value"] ~= 0 then
			replicated["Remotes"]["CommF_"]:InvokeServer("AddPoint", "Defense", L_44_[2])
		end
	elseif L_44_[3] == "سيف (Sword)" then
		if plr["Data"]["Points"]["Value"] ~= 0 then
			replicated["Remotes"]["CommF_"]:InvokeServer("AddPoint", "سيف (Sword)", L_44_[2])
		end
	elseif L_44_[3] == "مسدس (Gun)" then
		if plr["Data"]["Points"]["Value"] ~= 0 then
			replicated["Remotes"]["CommF_"]:InvokeServer("AddPoint", "مسدس (Gun)", L_44_[2])
		end
...
Useskills = function(L_58_arg0, L_59_arg1)
	...
	elseif L_60_[3] == "فاكهة بلوكس (Fruit)" then
		weaponSc("فاكهة بلوكس (Fruit)")
		if L_60_[2] == "Z" then
			vim1:SendKeyEvent(true, "Z", false, game)
			vim1:SendKeyEvent(false, "Z", false, game)
		elseif L_60_[2] == "X" then
...
LowCpu = function()
	...
	for L_71_forvar0, L_72_forvar1 in pairs(L_70_[2]:GetDescendants()) do
		...
		elseif L_73_[2]:IsA("Fire") or L_73_[2]:IsA("SpotLight") or L_73_[2]:IsA("دخان (Smoke)") or L_73_[2]:IsA("Sparkles") then
...
collectFruits = function(L_104_arg0)
	...
	if L_105_[1] then
		...
		for L_107_forvar0, L_108_forvar1 in pairs(workspace:GetChildren()) do
			...
			if string["find"](L_109_[2]["Name"], "فاكهة (Fruit)") then
...
DropFruits = function()
	for L_110_forvar0, L_111_forvar1 in next, plr["Backpack"]:GetChildren() do
		...
		if string["find"](L_112_[2]["Name"], "فاكهة (Fruit)") then
	...
	for L_113_forvar0, L_114_forvar1 in pairs(plr["Character"]:GetChildren()) do
		...
		if string["find"](L_115_[2]["Name"], "فاكهة (Fruit)") then
...
GetWP = function(L_128_arg0)
	...
	for L_130_forvar0, L_131_forvar1 in pairs(replicated["Remotes"]["CommF_"]:InvokeServer("getInventory")) do
		...
		if type(L_132_[1]) == "table" then
			if L_132_[1]["Type"] == "سيف (Sword)" then
...
function BTP(L_164_arg0)
	...
	repeat
		...
		L_165_[7]["مهمة (Quest)"]["Visible"] = false
...
QuestB = function()
	if World1 then
		if _G["FindBoss"] == "ملك الغوريلا (Gorilla King)" then
			bMon = "ملك الغوريلا (Gorilla King)"
			Qname = "JungleQuest"
			Qdata = 3
		...
		elseif _G["FindBoss"] == "بوبي (Bobby)" then
			bMon = "بوبي (Bobby)"
			Qname = "BuggyQuest1"
			Qdata = 3
		...
		elseif _G["FindBoss"] == "المنشار (The Saw)" then
			bMon = "المنشار (The Saw)"
			PosB = CFrame["new"](-784.89715576172, 72.427383422852, 1603.5822753906)
		elseif _G["FindBoss"] == "ييتي (Yeti)" then
			bMon = "ييتي (Yeti)"
			Qname = "SnowQuest"
			Qdata = 3
		...
		elseif _G["FindBoss"] == "زعيم الغوغاء (Mob Leader)" then
			bMon = "زعيم الغوغاء (Mob Leader)"
			PosB = CFrame["new"](-2844.7307128906, 7.4180502891541, 5356.6723632813)
		elseif _G["FindBoss"] == "نائب الأدميرال (Vice Admiral)" then
			bMon = "نائب الأدميرال (Vice Admiral)"
			Qname = "MarineQuest2"
			Qdata = 2
		...
		elseif _G["FindBoss"] == "خبير السيف (Saber Expert)" then
			bMon = "خبير السيف (Saber Expert)"
			PosB = CFrame["new"](-1458.89502, 29.8870335, -50.633564)
		elseif _G["FindBoss"] == "السجان (Warden)" then
			bMon = "السجان (Warden)"
			Qname = "ImpelQuest"
			Qdata = 1
		...
		elseif _G["FindBoss"] == "رئيس السجانين (Chief Warden)" then
			bMon = "رئيس السجانين (Chief Warden)"
			Qname = "ImpelQuest"
			Qdata = 2
			...
			PosQBoss = CFrame["new"](5191.86133, 2.84020686, 686.438721, -0.731384635, 0, .681965172, 0, 1, 0, -0.681965172, 0, -0.731384635)
		elseif _G["FindBoss"] == "سوان (Swan)" then
			bMon = "سوان (Swan)"
			Qname = "ImpelQuest"
			Qdata = 3
			PosB = CFrame["new"](5325.09619, 7.03906584, 719.570679, -0.309060812, 0, .951042235, 0, 1, 0, -0.951042235, 0, -0.309060812)
			PosQBoss = CFrame["new"](5191.86133, 2.84020686, 686.438721, -0.731384635, 0, .681965172, 0, 1, 0, -0.681965172, 0, -0.731384635)
		elseif _G["FindBoss"] == "أدميرال الماغما (Magma Admiral)" then
			bMon = "أدميرال الماغما (Magma Admiral)"
			Qname = "MagmaQuest"
			Qdata = 3
		...
		elseif _G["FindBoss"] == "سيد الرجل السمكة (Fishman Lord)" then
			bMon = "سيد الرجل السمكة (Fishman Lord)"
			Qname = "FishmanQuest"
			Qdata = 3
		...
		elseif _G["FindBoss"] == "ويسبير (Wysper)" then
			bMon = "ويسبير (Wysper)"
			Qname = "SkyExp1Quest"
			Qdata = 3
		...
		elseif _G["FindBoss"] == "إله الرعد (Thunder God)" then
			bMon = "إله الرعد (Thunder God)"
			Qname = "SkyExp2Quest"
			Qdata = 3
		...
		elseif _G["FindBoss"] == "سايبورغ (Cyborg)" then
			bMon = "سايبورغ (Cyborg)"
			Qname = "FountainQuest"
			Qdata = 3
		...
		elseif _G["FindBoss"] == "أدميرال الجليد (Ice Admiral)" then
			bMon = "أدميرال الجليد (Ice Admiral)"
			Qdata = nil
		...
		elseif _G["FindBoss"] == "اللحية الرمادية (Greybeard)" then
			bMon = "اللحية الرمادية (Greybeard)"
			Qdata = nil
	...
	if World2 then
		if _G["FindBoss"] == "ماس (Diamond)" then
			bMon = "ماس (Diamond)"
			Qname = "Area1Quest"
			Qdata = 3
		...
		elseif _G["FindBoss"] == "جيريمي (Jeremy)" then
			bMon = "جيريمي (Jeremy)"
			Qname = "Area2Quest"
			Qdata = 3
		...
		elseif _G["FindBoss"] == "دون سوان (Don Swan)" then
			bMon = "دون سوان (Don Swan)"
			PosB = CFrame["new"](2286.2004394531, 15.177839279175, 863.8388671875)
		elseif _G["FindBoss"] == "أدميرال الدخان (Smoke Admiral)" then
			bMon = "أدميرال الدخان (Smoke Admiral)"
			Qname = "IceSideQuest"
			Qdata = 3
		...
		elseif _G["FindBoss"] == "أدميرال الجليد المستيقظ (Ice Admiral)" then
			bMon = "أدميرال الجليد المستيقظ (Ice Admiral)"
			Qname = "FrostQuest"
			Qdata = 3
		...
		elseif _G["FindBoss"] == "حارس المد (Tide Keeper)" then
			bMon = "حارس المد (Tide Keeper)"
		...
		elseif _G["FindBoss"] == "اللحية السوداء (Darkbeard)" then
			bMon = "اللحية السوداء (Darkbeard)"
			Qdata = nil
	...
	if World3 then
		if _G["FindBoss"] == "ستون (Stone)" then
			bMon = "ستون (Stone)"
		...
		elseif _G["FindBoss"] == "زعيم الهيدرا (Hydra Leader)" then
			bMon = "زعيم الهيدرا (Hydra Leader)"
		...
		elseif _G["FindBoss"] == "أدميرال الكيلو (Kilo Admiral)" then
			bMon = "أدميرال الكيلو (Kilo Admiral)"
		...
		elseif _G["FindBoss"] == "القبطان الفيل (Captain Elephant)" then
			bMon = "القبطان الفيل (Captain Elephant)"
		...
		elseif _G["FindBoss"] == "القرصان الجميل (Beautiful Pirate)" then
			bMon = "القرصان الجميل (Beautiful Pirate)"
		...
		elseif _G["FindBoss"] == "ملكة الكعك (Cake Queen)" then
			bMon = "ملكة الكعك (Cake Queen)"
		...
		elseif _G["FindBoss"] == "لونغما (Longma)" then
			bMon = "لونغما (Longma)"
			Qdata = nil
		...
		elseif _G["FindBoss"] == "حاصد الأرواح (Soul Reaper)" then
			bMon = "حاصد الأرواح (Soul Reaper)"
			Qdata = nil
...
MaterialMon = function()
	...
	if World1 then
		if SelectMaterial == "أجنحة ملاك (Angel Wings)" then
			local L_177_ = {}
			MMon = {
				"Shanda",
				"Royal Squad",
		...
		elseif SelectMaterial == L_1_[2]({
			...
			SP = "Default"
		elseif SelectMaterial == "خام ماغما (Magma Ore)" then
			MMon = {
				"Military Soldier";
				"Military Spy";
				"أدميرال الماغما (Magma Admiral)"
			...
			SP = "Default"
		elseif SelectMaterial == "ذيل سمكة (Fish Tail)" then
			local L_178_ = {}
			MMon = {
				"Fishman Warrior",
				"Fishman Commando";
	...
	elseif World2 then
		...
		elseif SelectMaterial == "خام ماغما (Magma Ore)" then
			...
			SP = "Default"
		elseif SelectMaterial == "إكتوبلازم (Ectoplasm)" then
			local L_179_ = {}
			MMon = {
				"Ship Deckhand",
				"Ship Engineer",
		...
		elseif SelectMaterial == "قطرة غامضة (Mystic)" then
			MMon = {
				"Water Fighter"
			}
			MPos = CFrame["new"](-3385, 239, -10542)
			SP = "Default"
		elseif SelectMaterial == "مواد مشعة (Radioactive)" then
			MMon = {
				"Factory Staff"
			}
			MPos = CFrame["new"](295, 73, -56)
			SP = "Default"
		elseif SelectMaterial == "ناب مصاص دماء (Vampire)" then
			MMon = {
				"Vampire"
			}
	...
	elseif World3 then
		if SelectMaterial == "خردة معدنية (Scrap)" then
			MMon = {
				"Jungle Pirate",
				"Forest Pirate"
			}
		...
		elseif SelectMaterial == "ذيل سمكة (Fish Tail)" then
			...
			SP = "Default"
		elseif SelectMaterial == "كاكاو مستحضر (Cocoa)" then
			MMon = {
				L_1_[2]({
					"Chocolate Bar Battle",
					"r"
			...
			SP = "Default"
		elseif SelectMaterial == "حرشف تنين (Scale)" then
			MMon = {
				"Dragon Crew Archer";
				"Dragon Crew Warrior"
			}
			...
			SP = "Default"
		elseif SelectMaterial == "بارود (Gunpowder)" then
			MMon = {
				"Pistol Billionaire"
			}
			MPos = CFrame["new"](-84.855690002441, 85.620613098145, 6132.0087890625)
			SP = "Default"
		elseif SelectMaterial == "ناب صغير (Tusk)" then
			MMon = {
				"Mythological Pirate"
			}
			MPos = CFrame["new"](-13545, 470, -6917)
			SP = "Default"
		elseif SelectMaterial == "شعلة شيطانية (Wisp)" then
			MMon = {
				"Demonic Soul"
			}
...
L_1_[38] = L_1_[16]:MakeWindow({
	["Title"] = "STN Hub";
	["SubTitle"] = "v1.2 https://discord.gg/YDecu8Sfzt ",
...
-- زر نصي
L_1_[41] = Instance.new("TextButton")
L_1_[41].Size = UDim2.new(0, 50, 0, 50)
L_1_[41].Position = UDim2.new(0.15, 0, 0.15, 0)
L_1_[41].BackgroundColor3 = Color3.fromRGB(10, 40, 120) -- أزرق غامق
L_1_[41].Text = "SC"
L_1_[41].TextColor3 = Color3.fromRGB(255, 255, 255)
L_1_[41].TextScaled = true
L_1_[41].Font = Enum.Font.GothamBold
L_1_[41].Parent = L_1_[87]

-- تدوير الزر (دائري)
L_1_[82] = Instance.new("UICorner")
L_1_[82].CornerRadius = UDim.new(0.5, 0)
L_1_[82].Parent = L_1_[41]

-- إطار
L_1_[25] = Instance.new("UIStroke")
L_1_[25].Thickness = 2
L_1_[25].Color = Color3.fromRGB(0, 140, 255) -- أزرق
L_1_[25].ApplyStrokeMode = Enum.ApplyStrokeMode.Border
L_1_[25].Parent = L_1_[41]
...
L_1_[93] = {
	...
	["الإعدادات (Settings)"] = L_1_[38]:MakeTab({
		["Title"] = "Tab Setting",
		["Icon"] = L_1_[2]({
			"rbxassetid://7734053";
			"495"
...
spawn(function()
	...
	while wait(1) do
		local L_202_ = {}
		L_202_[1] = (game:GetService("ReplicatedStorage")):FindFirstChild("ريب إندرا (rip_indra)") or (game:GetService("Workspace"))["Enemies"]:FindFirstChild("rip_indra") and "✅️" or "❌️"
...
spawn(function()
	...
	while wait(1) do
		local L_204_ = {}
		L_204_[2] = (game:GetService("ReplicatedStorage")):FindFirstChild("ملك العجين (Dough King)") or (game:GetService("Workspace"))["Enemies"]:FindFirstChild("ملك العجين (Dough King)") and "✅️" or "❌️"
...
spawn(function()
	...
	while wait(1) do
		...
		L_206_[1] = (game:GetService("ReplicatedStorage"))["Remotes"]["CommF_"]:InvokeServer("EliteHunter", "التقدم (Progress)")
...
WeaponDropdown = Tabs["Main"]:CreateDropdown("اختر السلاح (Weapon)", {
		"اشتباك (Melee)";
		"سيف (Sword)",
		"فاكهة بلوكس (Fruit)";
		"مسدس (Gun)"
...
FarmLevel = Tabs["Main"]:CreateToggle("تطوير المستوى (Farm Level)", function(L_226_arg0)
		local L_227_ = {}
		L_227_[2] = L_226_arg0
		_G["Level"] = L_227_[2]
	...
	while task["wait"](Sec) do
		if _G["Level"] then
			pcall(function()
				...
				L_229_[2] = L_1_[112]()
				L_229_[1] = plr["PlayerGui"]["Main"]["مهمة (Quest)"]
...
ClosetMons = Tabs["Main"]:CreateToggle("Auto Farm Nearest", function(L_238_arg0)
		local L_239_ = {}
		L_239_[2] = L_238_arg0
...
FactoryRaids = Tabs["Main"]:CreateToggle("Auto Factory Raid", function(L_243_arg0)
		local L_244_ = {}
		L_244_[2] = L_243_arg0
...
CastleRaids = Tabs["Main"]:CreateToggle("Auto Pirate Raid", function(L_246_arg0)
		local L_247_ = {}
		L_247_[1] = L_246_arg0
...
Ecto = Tabs["Main"]:CreateToggle("Auto Farm Ectoplasm", function(L_258_arg0)
		local L_259_ = {}
		L_259_[1] = L_258_arg0
...
L_1_[125] = function()
	...
	pcall(function()
		if plr["PlayerGui"]["Main"]["مهمة (Quest)"]["Visible"] then
			replicated["Remotes"]["CommF_"]:InvokeServer("AbandonQuest")
			task["wait"](.5)
		end
...
Countdown = Tabs["Main"]:AddParagraph({
...
Tabs["Main"]:CreateToggle("Auto Farm Candy", function(L_292_arg0)
		local L_293_ = {}
		L_293_[1] = L_292_arg0
...
Tabs["Main"]:CreateDropdown("Select Island Gift", {
		"GiftMap1",
		"GiftMap2",
		"GiftMap3"
	}, function(L_304_arg0)
...
Tabs["Main"]:CreateToggle("Auto TP Gift", function(L_309_arg0)
		local L_310_ = {}
		L_310_[1] = L_309_arg0
		_G["AutoTPGift"] = L_310_[1]
	end)
Tabs["Main"]:CreateToggle("Auto Collect Gift", function(L_311_arg0)
		local L_312_ = {}
		L_312_[1] = L_311_arg0
...
Tabs["Main"]:CreateToggle("Auto Winter Gacha", function(L_317_arg0)
		local L_318_ = {}
		L_318_[2] = L_317_arg0
...
Tabs["Main"]:CreateToggle("Auto Open Gift", function(L_320_arg0)
		local L_321_ = {}
		L_321_[2] = L_320_arg0
...
Tabs["Main"]:CreateToggle("Auto UnStore Gift", function(L_333_arg0)
		local L_334_ = {}
		L_334_[2] = L_333_arg0
...
AutoStoreGift = Tabs["Main"]:CreateToggle("Auto Store Gift", function(L_336_arg0)
		local L_337_ = {}
		L_337_[2] = L_336_arg0
...
Tabs["Main"]:CreateButton("Buy 200 Frangments", function()
		pcall(function()
			local L_339_ = {}
			L_339_[1] = {
				[1] = "Candies";
...
Tabs["Main"]:CreateButton("Buy 500 Frangments", function()
		pcall(function()
			local L_340_ = {}
			L_340_[2] = {
				[1] = "Candies";
...
Tabs["Main"]:CreateButton("Buy Stats Refund", function()
		pcall(function()
			local L_342_ = {}
			L_342_[1] = {
				[1] = "Candies";
...
Tabs["Main"]:CreateButton("Buy Race Reroll", function()
		pcall(function()
			local L_343_ = {}
			L_343_[2] = {
				[1] = "Candies";
...
Tabs["Main"]:CreateButton("Buy Elf Hat", function()
		pcall(function()
			local L_344_ = {}
			L_344_[1] = {
				[1] = "Candies";
...
Tabs["Main"]:CreateButton("Buy Santa Hat", function()
		pcall(function()
			local L_345_ = {}
			L_345_[2] = {
				[1] = "Candies",
...
Tabs["Main"]:CreateButton("Buy Sleigh", function()
		pcall(function()
			local L_346_ = {}
			L_346_[2] = {
				[1] = "Candies";
...
ChestTW = Tabs["Main"]:CreateToggle("Auto Farm Chest", function(L_347_arg0)
		local L_348_ = {}
		L_348_[2] = L_347_arg0
...
ChestBP = Tabs["Main"]:CreateToggle("Auto Chest Bypass", function(L_352_arg0)
		local L_353_ = {}
		L_353_[1] = L_352_arg0
...
StopI = Tabs["Main"]:CreateToggle("Stop Items", function(L_367_arg0)
		local L_368_ = {}
		L_368_[1] = L_367_arg0
...
Berry = Tabs["Main"]:CreateToggle("Auto Farm Berry", function(L_369_arg0)
		local L_370_ = {}
		L_370_[2] = L_369_arg0
...
if World1 then
	Tabs["Main"]:CreateDropdown("Select Mob", {
			"Bandit";
			"Monkey";
			"Gorilla";
			"Pirate";
...
Tabs["Main"]:CreateToggle("Auto Kill Mob", function(L_405_arg0)
		local L_406_ = {}
		L_406_[2] = L_405_arg0
...
if World1 then
	Tabs["Main"]:CreateDropdown("Select Island", {
			"Pirates",
			"Marine",
			"Jungle";
			"Pirate Village",
...
Tabs["Main"]:CreateToggle("Auto Farm All Island", function(L_416_arg0)
		local L_417_ = {}
		L_417_[1] = L_416_arg0
...
spawn(function()
	while wait(Sec) do
		pcall(function()
			Process:SetDesc("Elite Procress :  " .. L_1_[18]["Remotes"]["CommF_"]:InvokeServer("EliteHunter", "التقدم (Progress)"))
		end)
	end
end)
...
spawn(function()
	...
	while wait(1) do
		...
		L_426_[1] = (game:GetService("ReplicatedStorage"))["Remotes"]["CommF_"]:InvokeServer("EliteHunter", "التقدم (Progress)")
...
EliteQ = Tabs["Main"]:CreateToggle("Auto Farm Elite", function(L_427_arg0)
		local L_428_ = {}
		L_428_[1] = L_427_arg0
	...
	while wait(1) do
		pcall(function()
			if _G["FarmEliteHunt"] then
				local L_429_ = {}
				L_429_[2] = L_1_[136]["PlayerGui"]["Main"]["مهمة (Quest)"]
	...
	while task["wait"](1) do
		pcall(function()
			if _G["FarmEliteH"] then
				local L_450_ = {}
				L_450_[2] = L_1_[136]["PlayerGui"]["Main"]["مهمة (Quest)"]
...
Tabs["Main"]:CreateToggle("Auto Unlocked Haki", function(L_466_arg0)
		local L_467_ = {}
		L_467_[1] = L_466_arg0
	...
	while task["wait"]() do
		if _G["Auto_Cake_Prince"] and not _G["AutoRaidCastle"] then
			pcall(function()
				...
				L_482_[3] = L_482_[4]["PlayerGui"]["Main"]["مهمة (Quest)"]
...
CakeQ = Tabs["Main"]:CreateToggle("Accept Quests", function(L_486_arg0)
		local L_487_ = {}
		L_487_[2] = L_486_arg0
	...
	while wait() do
		if _G["AutoDoughKing"] then
			pcall(function()
				...
				elseif GetConnectionEnemies("ملك العجين (Dough King)") then
					local L_493_ = {}
					L_493_[1] = GetConnectionEnemies("ملك العجين (Dough King)")
					if L_493_[1] then
						repeat
							task["wait"]()
				...
				if GetBP("God's Chalice") and GetM("كاكاو مستحضر (Cocoa)") >= 10 then
				...
				if GetM("كاكاو مستحضر (Cocoa)") <= 10 then
					local L_494_ = {}
...
Tabs["Main"]:CreateToggle("Auto Farm Dough King", function(L_495_arg0)
		local L_496_ = {}
		L_496_[2] = L_495_arg0
...
spawn(function()
	while wait() do
		if _G["AutoAttackDoughKing"] then
			pcall(function()
				local L_497_ = {}
				L_497_[1] = GetConnectionEnemies("ملك العجين (Dough King)")
				if L_497_[1] then
					repeat
						task["wait"]()
...
spawn(function()
	while task["wait"]() do
		if _G["AutoHop_Dough"] then
			pcall(function()
				local L_504_ = {}
				L_504_[2] = GetConnectionEnemies("ملك العجين (Dough King)")
				if L_504_[2] then
					repeat
						task["wait"]()
					...
					_tp(CFrame["new"](-1943.6765, 251.5095, -12337.8809))
					task["wait"](2)
					L_505_[1] = GetConnectionEnemies("ملك العجين (Dough King)")
	...
	while wait(Sec) do
		if _G["Doughv2"] then
			pcall(function()
				...
				elseif GetConnectionEnemies("ملك العجين (Dough King)") then
					local L_508_ = {}
					L_508_[2] = GetConnectionEnemies("ملك العجين (Dough King)")
					if L_508_[2] then
						repeat
							wait()
...
Tabs["Main"]:CreateToggle("Auto Farm Bone", function(L_512_arg0)
		local L_513_ = {}
		L_513_[1] = L_512_arg0
...
spawn(function()
	...
	while wait(.5) do
		...
		pcall(function()
			...
			L_515_[1] = L_514_[1]["PlayerGui"]:FindFirstChild("Main") and L_514_[1]["PlayerGui"]["Main"]:FindFirstChild("مهمة (Quest)")
...
BoneQ = Tabs["Main"]:CreateToggle("Accept Quests", function(L_517_arg0)
		local L_518_ = {}
		L_518_[1] = L_517_arg0
		_G["AcceptQuestB"] = L_518_[1]
	end)
Tabs["Main"]:CreateToggle("Auto Soul Reaper", function(L_519_arg0)
		local L_520_ = {}
		L_520_[2] = L_519_arg0
...
spawn(function()
	while wait(Sec) do
		if _G["AutoHytHallow"] then
			pcall(function()
				local L_521_ = {}
				L_521_[2] = GetConnectionEnemies("حاصد الأرواح (Soul Reaper)")
				if L_521_[2] then
					repeat
						task["wait"]()
					...
					if not GetBP("Hallow Essence") then
						repeat
							task["wait"](.1)
							L_1_[18]["Remotes"]["CommF_"]:InvokeServer("Bones", "شراء (Buy)", 1, 1)
...
RanBone = Tabs["Main"]:CreateToggle("Auto Random Bones", function(L_522_arg0)
		local L_523_ = {}
		L_523_[1] = L_522_arg0
...
Pray = Tabs["Main"]:CreateToggle("Auto Pray Gravestone", function(L_527_arg0)
		local L_528_ = {}
		L_528_[2] = L_527_arg0
		_G["Praying"] = L_528_[2]
	end)
...
spawn(function()
	pcall(function()
		while wait(1) do
			if workspace["Enemies"]:FindFirstChild("طاغية السماوات (Tyrant)") then
...
EyeStatus = Tabs["Main"]:AddParagraph({
...
FarmTyrant = Tabs["Main"]:CreateToggle("قتال الزعماء (Farm Boss)", function(L_536_arg0)
		local L_537_ = {}
		L_537_[2] = L_536_arg0
	...
	while wait(Sec) do
		if _G["FarmTyrant"] then
			pcall(function()
				...
				L_538_[4] = workspace["Enemies"]:FindFirstChild("طاغية السماوات (Tyrant)")
...
FarmPhaBinh = Tabs["Main"]:CreateToggle("Auto Summon Boss", function(L_546_arg0)
		local L_547_ = {}
		L_547_[2] = L_546_arg0
...
Toggle = Tabs["Main"]:CreateToggle("Auto Materials", function(L_564_arg0)
		local L_565_ = {}
		L_565_[1] = L_564_arg0;
...
FarmBoss = Tabs["Main"]:CreateToggle("قتال الزعماء (Farm Boss)", function(L_584_arg0)
		local L_585_ = {}
		L_585_[1] = L_584_arg0
			...
			while wait(Sec) do
				if _G["FarmBoss"] then
					pcall(function()
						...
						L_586_[2] = L_1_[136]["PlayerGui"]["Main"]["مهمة (Quest)"]["Container"]["QuestTitle"]["Title"]["Text"]
						if _G["AcceptQuestBoss"] and L_586_[1] then
							if not string["find"](L_586_[2], (QuestBeta())[0]) then
								L_1_[18]["Remotes"]["CommF_"]:InvokeServer("AbandonQuest")
							end)
BossQ = Tabs["Main"]:CreateToggle("Accept Quests", function(L_593_arg0)
		local L_594_ = {}
		L_594_[2] = L_593_arg0
		_G["AcceptQuestBoss"] = L_594_[2]
	end)
FarmAllBoss = Tabs["Main"]:CreateToggle("Auto Farm All Boss", function(L_595_arg0)
		local L_596_ = {}
		L_596_[1] = L_595_arg0
...
L_1_[74] = Tabs["Main"]:CreateDropdown({
	["Name"] = "Choose Island";
	["Description"] = "",
	["Options"] = L_1_[17],
	["Default"] = false;
...
L_1_[137] = Tabs["Main"]:CreateToggle("Auto Mastery Fruits", function(L_606_arg0)
		local L_607_ = {}
		L_607_[2] = L_606_arg0
...
L_1_[46] = Tabs["Main"]:CreateToggle("Auto Mastery Gun", function(L_613_arg0)
		local L_614_ = {}
		L_614_[1] = L_613_arg0
				...
				if SelectIsland == "Cake" then
					...
					if L_615_[2] then
						...
						repeat
							...
							if (L_1_[136]["Character"]:FindFirstChildOfClass("Tool"))["ToolTip"] ~= "مسدس (Gun)" then
								return
							end
...
L_1_[29] = Tabs["Main"]:CreateToggle({
	["Name"] = L_1_[2]({
				...
				if SelectIsland == "Cake" then
					for L_621_forvar0, L_622_forvar1 in next, L_1_[18]["Remotes"]["CommF_"]:InvokeServer("getInventory") do
						...
						if type(L_623_[2]) == "table" then
							if L_623_[2]["Type"] == "سيف (Sword)" then
								SwordName = L_623_[2]["Name"]
								if tonumber(L_623_[2]["إتقان (Mastery)"]) >= 1 or tonumber(L_623_[2]["إتقان (Mastery)"]) <= 599 then
									local L_624_ = {}
									...
									if GetBP(SwordName) then
										if L_624_[2] then
											repeat
												wait()
												L_1_[4]["سيف (Sword)"](L_624_[2], _G["FarmMastery_S"])
								...
								elseif tonumber(L_623_[2]["إتقان (Mastery)"]) >= 600 then
									if GetBP(SwordName) then
										return nil
									else
										L_1_[18]["Remotes"]["CommF_"]:InvokeServer("LoadItem", SwordName)
				...
				elseif SelectIsland == "Bone" then
					for L_625_forvar0, L_626_forvar1 in next, L_1_[18]["Remotes"]["CommF_"]:InvokeServer("getInventory") do
						...
						if type(L_627_[1]) == "table" then
							if L_627_[1]["Type"] == "سيف (Sword)" then
								SwordName = L_627_[1]["Name"]
								if tonumber(L_627_[1]["إتقان (Mastery)"]) >= 1 or tonumber(L_627_[1]["إتقان (Mastery)"]) <= 599 then
									local L_628_ = {}
									...
									if GetBP(SwordName) then
										if L_628_[2] then
											repeat
												wait()
												L_1_[4]["سيف (Sword)"](L_628_[2], _G["FarmMastery_S"])
								...
								elseif tonumber(L_627_[1]["إتقان (Mastery)"]) >= 600 then
									if GetBP(SwordName) then
										return nil
									else
										L_1_[18]["Remotes"]["CommF_"]:InvokeServer("LoadItem", SwordName)
...
Tabs["الإعدادات (Settings)"]:AddSection({
	"Settings / Configure"
})
Initialize = Tabs["الإعدادات (Settings)"]:CreateToggle("هجوم سريع (Fast)", function(L_629_arg0)
		local L_630_ = {}
		L_630_[2] = L_629_arg0
		_G["Seriality"] = L_630_[2]
	end)
Bringmob = Tabs["الإعدادات (Settings)"]:CreateToggle("Bring Mobs", function(L_631_arg0)
		local L_632_ = {}
		L_632_[2] = L_631_arg0
		_B = L_632_[2]
	end)
Tabs["الإعدادات (Settings)"]:CreateDropdown("Bring Mob Mode", {
		"250";
		"300";
		"350",
		"400";
...
Tabs["الإعدادات (Settings)"]:CreateDropdown("Bring Mob Speed", {
		"250";
		"300",
		"350";
		"400";
...
Tabs["الإعدادات (Settings)"]:CreateDropdown("Bring Mob Max", {
		"2";
		"3";
		"4";
		"5";
...
Tabs["الإعدادات (Settings)"]:CreateToggle("Auto Hop Server", function(L_639_arg0)
		local L_640_ = {}
		L_640_[2] = L_639_arg0
...
Tabs["الإعدادات (Settings)"]:CreateSlider("Hop Delay (Minutes)", 5, 120, 30, function(L_641_arg0)
		local L_642_ = {}
		L_642_[1] = L_641_arg0
		_G["HopDelay"] = L_642_[1] * 60
	end)
Tabs["الإعدادات (Settings)"]:CreateToggle("Auto Set Spawn Point", function(L_643_arg0)
		local L_644_ = {}
		L_644_[1] = L_643_arg0;
...
BusuAura = Tabs["الإعدادات (Settings)"]:CreateToggle("Auto Turn on Buso", function(L_645_arg0)
		local L_646_ = {}
		L_646_[1] = L_645_arg0
		Boud = L_646_[1]
	end)
...
Tabs["الإعدادات (Settings)"]:CreateToggle({
	["Name"] = L_1_[2]({
...
RaceV3Aura = Tabs["الإعدادات (Settings)"]:CreateToggle("Auto Turn on Race V3", function(L_650_arg0)
		local L_651_ = {}
		L_651_[1] = L_650_arg0
...
RaceV4Aura = Tabs["الإعدادات (Settings)"]:CreateToggle("Auto Turn on Race V4", function(L_652_arg0)
		local L_653_ = {}
		L_653_[2] = L_652_arg0
...
Tabs["الإعدادات (Settings)"]:CreateToggle("Auto Speed", function(L_656_arg0)
		local L_657_ = {}
		L_657_[2] = L_656_arg0
...
Tabs["الإعدادات (Settings)"]:CreateSlider("Speed Value", 26, 300, (getgenv())["WalkSpeedValue"], function(L_658_arg0)
		local L_659_ = {}
		L_659_[3] = L_658_arg0;
...
Tabs["الإعدادات (Settings)"]:CreateToggle("Auto Jump", function(L_663_arg0)
		local L_664_ = {}
		L_664_[2] = L_663_arg0
...
Tabs["الإعدادات (Settings)"]:CreateSlider("Jump Value", 50, 500, (getgenv())["JumpValue"], function(L_666_arg0)
		local L_667_ = {}
		L_667_[1] = L_666_arg0;
...
RandomAround = Tabs["الإعدادات (Settings)"]:CreateToggle({
	["Name"] = L_1_[2]({
...
Byp = Tabs["الإعدادات (Settings)"]:CreateToggle({
	["Name"] = L_1_[2]({
...
SafeModes = Tabs["الإعدادات (Settings)"]:CreateToggle("Panic Mode", function(L_673_arg0)
		local L_674_ = {}
		L_674_[1] = L_673_arg0
...
DisableHitVFX = Tabs["الإعدادات (Settings)"]:CreateToggle({
	["Name"] = "Remove Hit VFX";
	["Description"] = L_1_[2]({
...
RmvVFX = Tabs["الإعدادات (Settings)"]:CreateToggle({
	["Name"] = L_1_[2]({
		"Remove Death & Respa",
		"wned VFX"
	});
...
DisblesNotify = Tabs["الإعدادات (Settings)"]:CreateToggle("Disable Notify", function(L_683_arg0)
		local L_684_ = {}
		L_684_[2] = L_683_arg0
...
Tabs["الإعدادات (Settings)"]:CreateToggle("Anti AFK", function(L_685_arg0)
		local L_686_ = {}
		L_686_[1] = L_685_arg0
		if L_686_[1] then
			local L_687_ = {}
...
Tabs["الإعدادات (Settings)"]:CreateToggle("No Clip", function(L_693_arg0)
		local L_694_ = {}
		L_694_[1] = L_693_arg0;
...
StatusSelect = Tabs["Esp"]:CreateSlider("Stats Value", 0, 1000, 10, function(L_698_arg0)
		local L_699_ = {}
		L_699_[2] = L_698_arg0
		pSats = L_699_[2]
	end)
StatsUpg = Tabs["Esp"]:CreateToggle("Auto Melee", function(L_700_arg0)
		local L_701_ = {}
		L_701_[2] = L_700_arg0
	...
	while wait(Sec) do
		pcall(function()
			if _G["Auto_Melee"] then
				statsSetings("اشتباك (Melee)", pSats)
			end
		end)
	end
end)
StatsUpg = Tabs["Esp"]:CreateToggle("Auto Swords", function(L_702_arg0)
		local L_703_ = {}
		L_703_[2] = L_702_arg0
	...
	while wait(Sec) do
		pcall(function()
			if _G["Auto_Sword"] then
				statsSetings("سيف (Sword)", pSats)
			end
		end)
	end
end)
StatsUpg = Tabs["Esp"]:CreateToggle("Auto Gun", function(L_704_arg0)
		local L_705_ = {}
		L_705_[1] = L_704_arg0
...
spawn(function()
	while wait(Sec) do
		pcall(function()
			if _G["Auto_Gun"] then
				statsSetings("مسدس (Gun)", pSats)
			end
		end)
	end
end)
StatsUpg = Tabs["Esp"]:CreateToggle("Auto Blox Fruit", function(L_706_arg0)
		local L_707_ = {}
		L_707_[2] = L_706_arg0
...
StatsUpg = Tabs["Esp"]:CreateToggle("Auto Defense", function(L_708_arg0)
		local L_709_ = {}
		L_709_[1] = L_708_arg0
...
Tabs["Fish"]:AddSection({
	"Fishing"
})
Tabs["Fish"]:CreateDropdown("Select Fishing Rod", {
		"Fishing Rod";
		"Gold Rod";
		"Shark Rod",
		"Shell Rod",
...
FishingToggle = Tabs["Fish"]:CreateToggle("Auto Fishing", function(L_716_arg0)
		local L_717_ = {}
		L_717_[2] = L_716_arg0
...
FishingQ = Tabs["Fish"]:CreateToggle("Auto Quest Fishing", function(L_721_arg0)
		local L_722_ = {}
		L_722_[2] = L_721_arg0
...
L_1_[31] = function()
	local L_723_ = {}
	L_723_[2] = L_1_[68]["PlayerGui"]:FindFirstChild("مهمة (Quest)") or L_1_[68]["PlayerGui"]:FindFirstChild("QuestGui")
...
QuestToggle = Tabs["Fish"]:CreateToggle("Auto Complete Quest", function(L_724_arg0)
		local L_725_ = {}
		L_725_[2] = L_724_arg0
...
SellFishToggle = Tabs["Fish"]:CreateToggle("Auto Sell Fish", function(L_726_arg0)
		local L_727_ = {}
		L_727_[2] = L_726_arg0
...
SpamSkillZ = Tabs["Fish"]:CreateToggle("Auto Spam Skill Z", function(L_728_arg0)
		local L_729_ = {}
		L_729_[1] = L_728_arg0
...
TravelDress = Tabs["Quests"]:CreateToggle("Auto Quest Sea 2", function(L_730_arg0)
		local L_731_ = {}
		L_731_[2] = L_730_arg0
		...
		pcall(function()
			if _G["TravelDres"] then
				if L_1_[45]["Data"]["Level"]["Value"] >= 700 then
					if workspace["Map"]["جليد (Ice)"]["Door"]["CanCollide"] == true and workspace["Map"]["جليد (Ice)"]["Door"]["Transparency"] == 0 then
					...
					elseif workspace["Map"]["جليد (Ice)"]["Door"]["CanCollide"] == false and workspace["Map"]["جليد (Ice)"]["Door"]["Transparency"] == 1 then
						if Enemies:FindFirstChild("أدميرال الجليد (Ice Admiral)") then
							for L_732_forvar0, L_733_forvar1 in pairs(Enemies:GetChildren()) do
								local L_734_ = {}
								L_734_[1], L_734_[2] = L_732_forvar0, L_733_forvar1
								if L_734_[2]["Name"] == "أدميرال الجليد (Ice Admiral)" and L_1_[4]["Alive"](L_734_[2]) then
									repeat
										task["wait"]()
...
Zou = Tabs["Quests"]:CreateToggle("Auto Quest Sea 3", function(L_735_arg0)
		local L_736_ = {}
		L_736_[2] = L_735_arg0
		_G["AutoZou"] = L_736_[2]
	end)
					...
					if L_1_[18]["Remotes"]["CommF_"]:InvokeServer("BartiloQuestProgress", "Bartilo") == 3 then
						if (L_1_[18]["Remotes"]["CommF_"]:InvokeServer("GetUnlockables"))["FlamingoAccess"] ~= nil then
							...
							elseif L_1_[18]["Remotes"]["CommF_"]:InvokeServer("ZQuestProgress", "Check") == 1 then
								...
								local L_738_ = {}
								L_738_[1] = GetConnectionEnemies("دون سوان (Don Swan)")
								if L_738_[1] then
									repeat
										wait()
						...
						if L_1_[18]["Remotes"]["CommF_"]:InvokeServer("BartiloQuestProgress", "Bartilo") == 0 then
							if string["find"](L_1_[45]["PlayerGui"]["Main"]["مهمة (Quest)"]["Container"]["QuestTitle"]["Title"]["Text"], "Swan Pirates") and (string["find"](L_1_[45]["PlayerGui"]["Main"]["مهمة (Quest)"]["Container"]["QuestTitle"]["Title"]["Text"], "50") and L_1_[45]["PlayerGui"]["Main"]["مهمة (Quest)"]["Visible"] == true) then
								local L_754_ = {}
								...
								if L_754_[1] then
									pcall(function()
										...
										until not L_754_[1]["Parent"] or L_754_[1]["Humanoid"]["Health"] <= 0 or _G["AutoZou"] == false or L_1_[45]["PlayerGui"]["Main"]["مهمة (Quest)"]["Visible"] == false
									end)
								else
						...
						elseif L_1_[18]["Remotes"]["CommF_"]:InvokeServer("BartiloQuestProgress", "Bartilo") == 1 then
							local L_755_ = {}
							L_755_[2] = GetConnectionEnemies("جيريمي (Jeremy)")
							if L_755_[2] then
								repeat
									wait()
...
Tabs["Quests"]:AddSection({
	"Tushita + Yama"
})
Q = Tabs["Quests"]:CreateToggle("Auto Tushita Sword", function(L_756_arg0)
		local L_757_ = {}
		L_757_[2] = L_756_arg0
		...
		pcall(function()
			if _G["Auto_Tushita"] then
				if workspace["Map"]["Turtle"]:FindFirstChild("TushitaGate") then
					...
					local L_758_ = {}
					L_758_[1] = GetConnectionEnemies("لونغما (Longma)")
					if L_758_[1] then
						repeat
							task["wait"]()
						...
						if L_1_[18]:FindFirstChild("لونغما (Longma)") then
							_tp((L_1_[18]:FindFirstChild("لونغما (Longma)"))["HumanoidRootPart"]["CFrame"] * CFrame["new"](0, 40, 0))
						end
					end
				end
			end
...
Q = Tabs["Quests"]:CreateToggle("Auto Yama Sword", function(L_759_arg0)
		local L_760_ = {}
		L_760_[2] = L_759_arg0
	...
	while wait(Sec) do
		pcall(function()
			if _G["Auto_Yama"] then
				if L_1_[18]["Remotes"]["CommF_"]:InvokeServer("EliteHunter", "التقدم (Progress)") < 30 then
					_G["FarmEliteHunt"] = true
				elseif L_1_[18]["Remotes"]["CommF_"]:InvokeServer("EliteHunter", "التقدم (Progress)") > 30 then
					_G["FarmEliteHunt"] = false
					if (workspace["Map"]["Waterfall"]["SealedKatana"]["Handle"]["Position"] - L_1_[45]["Character"]["HumanoidRootPart"]["Position"])["Magnitude"] >= 20 then
						...
						L_761_[1] = GetConnectionEnemies("شبح (Ghost)")
						if L_761_[1] then
							repeat
								wait()
...
CheckSoul = Tabs["Quests"]:AddParagraph({
	["Title"] = L_1_[2]({
		" Skull Guitar Quests",
		" "
	}),
...
Tabs["Quests"]:CreateToggle("Auto Skull Guitar", function(L_762_arg0)
		local L_763_ = {}
		L_763_[2] = L_762_arg0
...
function getRT(L_767_arg0)
	local L_768_ = {}
	L_768_[1] = L_767_arg0
	L_768_[2] = workspace["Map"]["Haunted Castle"]["Trophies"]["مهمة (Quest)"]
		...
		pcall(function()
			if _G["Auto_Soul_Guitar"] then
				if World3 then
					...
					elseif (L_1_[18]["Remotes"]["CommF_"]:InvokeServer("GuitarPuzzleProgress", "Check"))["شبح (Ghost)"] == false then
						L_1_[18]["Remotes"]["CommF_"]:InvokeServer("GuitarPuzzleProgress", "شبح (Ghost)")
						L_1_[18]["Remotes"]["CommF_"]:InvokeServer("GuitarPuzzleProgress", "شبح (Ghost)", true)
	...
	while wait(Sec) do
		pcall(function()
			if _G["AutoMatSoul"] and GetWP("Skull Guitar") == false then
				if GetM("Bones") >= 500 and (GetM("إكتوبلازم (Ectoplasm)") >= 250 and GetM("Dark Fragment") >= 1) then
					L_1_[18]["Remotes"]["CommF_"]:InvokeServer("soulGuitarBuy", true)
				else
					if GetM("إكتوبلازم (Ectoplasm)") <= 250 then
					...
					elseif GetM("Dark Fragment") < 1 then
						if _G["AutoMatSoul"] and World2 then
							local L_784_ = {}
							L_784_[1] = GetConnectionEnemies("اللحية السوداء (Darkbeard)")
							if L_784_[1] then
								repeat
									task["wait"]()
						...
						if not GetConnectionEnemies("اللحية السوداء (Darkbeard)") then
							Hop()
						end
...
spawn(function()
	while wait(Sec) do
		pcall(function()
			if _G["CDK"] then
				local L_788_ = {}
				L_1_[18]["Remotes"]["CommF_"]:InvokeServer("CDKQuest", "التقدم (Progress)", "Good")
				L_1_[18]["Remotes"]["CommF_"]:InvokeServer("CDKQuest", "التقدم (Progress)", "Evil")
...
Q = Tabs["Quests"]:CreateToggle("Auto Yama CDK", function(L_789_arg0)
		local L_790_ = {}
		L_790_[2] = L_789_arg0
		_G["CDK_YM"] = L_790_[2]
	end)
		...
		pcall(function()
			if _G["CDK_YM"] then
				if tostring(L_1_[18]["Remotes"]["CommF_"]:InvokeServer("CDKQuest", "OpenDoor")) ~= "opened" then
					...
					if (L_1_[18]["Remotes"]["CommF_"]:InvokeServer("CDKQuest", "التقدم (Progress)"))["Finished"] == nil then
						L_1_[18]["Remotes"]["CommF_"]:InvokeServer("CDKQuest", "StartTrial", "Evil")
						L_1_[18]["Remotes"]["CommF_"]:InvokeServer("CDKQuest", "StartTrial", "Evil")
					elseif (L_1_[18]["Remotes"]["CommF_"]:InvokeServer("CDKQuest", "التقدم (Progress)"))["Finished"] == false then
						if tonumber((L_1_[18]["Remotes"]["CommF_"]:InvokeServer("CDKQuest", "التقدم (Progress)"))["Evil"]) == -3 then
							QuestYama_1 = true
							QuestYama_2 = false
							QuestYama_3 = false
							repeat
							...
							until tonumber((L_1_[18]["Remotes"]["CommF_"]:InvokeServer("CDKQuest", "التقدم (Progress)"))["Evil"]) == 1 or not _G["CDK_YM"]
						elseif tonumber((L_1_[18]["Remotes"]["CommF_"]:InvokeServer("CDKQuest", "التقدم (Progress)"))["Evil"]) == -4 then
							QuestYama_1 = false
							QuestYama_2 = true
							QuestYama_3 = false
										...
										if (L_797_[2]["Position"] - Root["Position"])["Magnitude"] <= 1000 and workspace["Enemies"]:FindFirstChild(L_797_[1]) then
											for L_798_forvar0, L_799_forvar1 in pairs(workspace["Enemies"]:GetChildren()) do
												...
												if L_800_[2]:FindFirstChild("HumanoidRootPart") and (L_800_[2]:FindFirstChild("Humanoid") and ((L_800_[2]:FindFirstChild("Humanoid"))["Health"] > 0 and L_800_[2]:FindFirstChild("HazeESP"))) then
													...
													until not _G["CDK_YM"] or tonumber((L_1_[18]["Remotes"]["CommF_"]:InvokeServer("CDKQuest", "التقدم (Progress)"))["Evil"]) == 2 or not L_800_[2]:FindFirstChild("HazeESP") or L_800_[2]["Humanoid"]["Health"] <= 0
												end
											end
										else
											_tp(L_797_[2])
						...
						elseif tonumber((L_1_[18]["Remotes"]["CommF_"]:InvokeServer("CDKQuest", "التقدم (Progress)"))["Evil"]) == -5 then
							QuestYama_1 = false
							QuestYama_2 = false
							QuestYama_3 = true
								...
								if (Root["Position"] - workspace["Map"]["HellDimension"]["Spawn"]["Position"])["Magnitude"] <= 1000 then
									for L_801_forvar0, L_802_forvar1 in pairs(workspace["Map"]["HellDimension"]["Exit"]:GetChildren()) do
										...
										if tonumber(L_803_[1]) == 2 then
											...
											until not _G["CDK_YM"] or tonumber((L_1_[18]["Remotes"]["CommF_"]:InvokeServer("CDKQuest", "التقدم (Progress)"))["Evil"]) == 3
										end
									end
									EquipWeapon(_G["SelectWeapon"])
									if tonumber((L_1_[18]["Remotes"]["CommF_"]:InvokeServer("CDKQuest", "التقدم (Progress)"))["Evil"]) ~= 3 then
										repeat
											...
											wait(2)
											_G["T1Yama"] = true
										until not _G["CDK_YM"] or _G["T1Yama"] or tonumber((L_1_[18]["Remotes"]["CommF_"]:InvokeServer("CDKQuest", "التقدم (Progress)"))["Evil"]) == 3
										repeat
											task["wait"]()
											repeat
												task["wait"]()
											...
											wait(2)
											_G["T2Yama"] = true
										until _G["T2Yama"] or _G["CDK_YM"] == false or tonumber((L_1_[18]["Remotes"]["CommF_"]:InvokeServer("CDKQuest", "التقدم (Progress)"))["Evil"]) == 3
										repeat
											wait()
											repeat
												task["wait"]()
											...
											wait(2)
											_G["T3Yama"] = true
										until _G["T3Yama"] or _G["CDK_YM"] == false or tonumber((L_1_[18]["Remotes"]["CommF_"]:InvokeServer("CDKQuest", "التقدم (Progress)"))["Evil"]) == 3
									end
									for L_813_forvar0, L_814_forvar1 in pairs(workspace["Enemies"]:GetChildren()) do
										...
										if ((L_815_[3]:FindFirstChild("HumanoidRootPart"))["Position"] - workspace["Map"]["HellDimension"]["Spawn"]["Position"])["Magnitude"] <= 300 then
											if L_815_[3]:FindFirstChild("HumanoidRootPart") and (L_815_[3]:FindFirstChild("Humanoid") and (L_815_[3]:FindFirstChild("Humanoid"))["Health"] > 0) then
												...
												until not _G["CDK_YM"] or L_815_[3]["Humanoid"]["Health"] <= 0 or not L_815_[3]["Parent"] or tonumber((L_1_[18]["Remotes"]["CommF_"]:InvokeServer("CDKQuest", "التقدم (Progress)"))["Evil"]) == 3
											end
										end
									end
								end
...
spawn(function()
	while wait() do
		pcall(function()
			if _G["CDK_YM"] then
				if tonumber((L_1_[18]["Remotes"]["CommF_"]:InvokeServer("CDKQuest", "التقدم (Progress)"))["Evil"]) == -5 then
					if not workspace["Map"]:FindFirstChild("HellDimension") or (Root["Position"] - workspace["Map"]["HellDimension"]["Spawn"]["Position"])["Magnitude"] > 1000 then
						local L_816_ = {}
						L_816_[2] = GetConnectionEnemies("حاصد الأرواح (Soul Reaper)")
						if L_816_[2] then
							repeat
								task["wait"]()
								_tp(L_816_[2]["HumanoidRootPart"]["CFrame"])
							until L_816_[2]["Humanoid"]["Health"] <= 0 or not _G["CDK_YM"] or not L_816_[2]["Parent"] or tonumber((L_1_[18]["Remotes"]["CommF_"]:InvokeServer("CDKQuest", "التقدم (Progress)"))["Evil"]) == 3 or workspace["Map"]:FindFirstChild("HellDimension") and (Root["Position"] - workspace["Map"]["HellDimension"]["Spawn"]["Position"])["Magnitude"] <= 1000
						elseif L_1_[45]["Backpack"]:FindFirstChild("Hallow Essence") or L_1_[45]["Character"]:FindFirstChild("Hallow Essence") then
							...
							EquipWeapon("Hallow Essence")
						elseif L_1_[18]:FindFirstChild("حاصد الأرواح (Soul Reaper)") and (L_1_[18]:FindFirstChild("حاصد الأرواح (Soul Reaper)"))["Humanoid"]["Health"] > 0 then
							_tp((L_1_[18]:FindFirstChild("حاصد الأرواح (Soul Reaper)"))["HumanoidRootPart"]["CFrame"])
						else
							if L_1_[18]["Remotes"]["CommF_"]:InvokeServer("Bones", "Check") < 50 and (not workspace["Enemies"]:FindFirstChild("حاصد الأرواح (Soul Reaper)") and (not L_1_[18]:FindFirstChild("حاصد الأرواح (Soul Reaper)") and not workspace["Map"]:FindFirstChild("HellDimension"))) then
...
Q = Tabs["Quests"]:CreateToggle("Auto Tushita CDK", function(L_820_arg0)
		local L_821_ = {}
		L_821_[1] = L_820_arg0
		_G["CDK_TS"] = L_821_[1]
	end)
		...
		pcall(function()
			if _G["CDK_TS"] then
				if tostring(L_1_[18]["Remotes"]["CommF_"]:InvokeServer("CDKQuest", "OpenDoor")) ~= "opened" then
					...
					elseif (L_1_[18]["Remotes"]["CommF_"]:InvokeServer("CDKQuest", "التقدم (Progress)"))["Finished"] == false then
						if tonumber((L_1_[18]["Remotes"]["CommF_"]:InvokeServer("CDKQuest", "التقدم (Progress)"))["Good"]) == -3 then
							QuestTushita_1 = true
							...
							until ((CFrame["new"](-4602.5107421875, 16.446542739868, -2880.998046875))["Position"] - (game:GetService("Players"))["LocalPlayer"]["Character"]["HumanoidRootPart"]["Position"])["Magnitude"] <= 3 or not _G["CDK_TS"] or tonumber((L_1_[18]["Remotes"]["CommF_"]:InvokeServer("CDKQuest", "التقدم (Progress)"))["Good"]) == 1
							...
							until ((CFrame["new"](4001.1853027344, 10.089399337769, -2654.86328125))["Position"] - (game:GetService("Players"))["LocalPlayer"]["Character"]["HumanoidRootPart"]["Position"])["Magnitude"] <= 3 or not _G["CDK_TS"] or tonumber((L_1_[18]["Remotes"]["CommF_"]:InvokeServer("CDKQuest", "التقدم (Progress)"))["Good"]) == 1
							...
							until ((CFrame["new"](-9530.763671875, 7.2452087402344, -8375.5087890625))["Position"] - (game:GetService("Players"))["LocalPlayer"]["Character"]["HumanoidRootPart"]["Position"])["Magnitude"] <= 3 or not _G["CDK_TS"] or tonumber((L_1_[18]["Remotes"]["CommF_"]:InvokeServer("CDKQuest", "التقدم (Progress)"))["Good"]) == 1
							...
							wait(1)
						elseif tonumber((L_1_[18]["Remotes"]["CommF_"]:InvokeServer("CDKQuest", "التقدم (Progress)"))["Good"]) == -4 then
							...
							until not _G["CDK_TS"] or tonumber((L_1_[18]["Remotes"]["CommF_"]:InvokeServer("CDKQuest", "التقدم (Progress)"))["Good"]) == 2
							_G["AutoRaidCastle"] = false
						elseif tonumber((L_1_[18]["Remotes"]["CommF_"]:InvokeServer("CDKQuest", "التقدم (Progress)"))["Good"]) == -5 then
							QuestTushita_1 = false
							QuestTushita_2 = false
							QuestTushita_3 = true
							if workspace["Enemies"]:FindFirstChild("ملكة الكعك (Cake Queen)") then
								for L_822_forvar0, L_823_forvar1 in pairs(workspace["Enemies"]:GetChildren()) do
									local L_824_ = {}
									L_824_[1], L_824_[3] = L_822_forvar0, L_823_forvar1
									if L_824_[3]["Name"] == "ملكة الكعك (Cake Queen)" then
										if L_824_[3]:FindFirstChild("Humanoid") and (L_824_[3]:FindFirstChild("HumanoidRootPart") and L_824_[3]["Humanoid"]["Health"] > 0) then
											...
											until not _G["CDK_TS"] or not L_824_[3]["Parent"] or L_824_[3]["Humanoid"]["Health"] <= 0 or tonumber((L_1_[18]["Remotes"]["CommF_"]:InvokeServer("CDKQuest", "التقدم (Progress)"))["Good"]) == 3
										end
									end
								end
							elseif L_1_[18]:FindFirstChild("ملكة الكعك (Cake Queen)") and (L_1_[18]:FindFirstChild("ملكة الكعك (Cake Queen)"))["Humanoid"]["Health"] > 0 then
								_tp((L_1_[18]:FindFirstChild("ملكة الكعك (Cake Queen)"))["HumanoidRootPart"]["CFrame"] * CFrame["new"](0, 30, 0))
							else
								if (game["Players"]["LocalPlayer"]["Character"]["HumanoidRootPart"]["Position"] - workspace["Map"]["HeavenlyDimension"]["Spawn"]["Position"])["Magnitude"] <= 1000 then
									...
									if Ex == 2 then
										...
										until not _G["CDK_TS"] or tonumber((L_1_[18]["Remotes"]["CommF_"]:InvokeServer("CDKQuest", "التقدم (Progress)"))["Good"]) == 3
									end
									repeat
										wait()
										repeat
...
Tabs["Quests"]:CreateButton("Buy Legendary Sword", function()
		L_1_[18]["Remotes"]["CommF_"]:InvokeServer("Legend)
...
Q = Tabs["Quests"]:CreateToggle("Auto Pole V1", function(L_845_arg0)
		local L_846_ = {}
		L_846_[1] = L_845_arg0
...
spawn(function()
	while wait(Sec) do
		if _G["AutoPole"] then
			pcall(function()
				local L_847_ = {}
				L_847_[2] = GetConnectionEnemies("إله الرعد (Thunder God)")
				if L_847_[2] then
					repeat
						task["wait"]()
...
Q = Tabs["Quests"]:CreateToggle("Auto Pole V2 [Beta]", function(L_848_arg0)
		local L_849_ = {}
		L_849_[1] = L_848_arg0
		...
		pcall(function()
			if _G["AutoPoleV2"] then
				...
				if (GetBP("Rumble Fruit"))["AwakenedMoves"]:FindFirstChild("Z") and ((GetBP("Rumble Fruit"))["AwakenedMoves"]:FindFirstChild("X") and ((GetBP("Rumble Fruit"))["AwakenedMoves"]:FindFirstChild("C") and ((GetBP("Rumble Fruit"))["AwakenedMoves"]:FindFirstChild("V") and (GetBP("Rumble Fruit"))["AwakenedMoves"]:FindFirstChild("F")))) then
					...
					if L_1_[45]["Data"]["شظايا (Frag)"]["Value"] >= 5000 then
						L_1_[18]["Remotes"]["CommF_"]:InvokeServer("إله الرعد (Thunder God)", "Talk")
						wait(Sec)
						L_1_[18]["Remotes"]["CommF_"]:InvokeServer("إله الرعد (Thunder God)", "Sure")
					end
...
Tabs["Quests"]:CreateToggle("Auto Saw Sword", function(L_851_arg0)
		local L_852_ = {}
		L_852_[1] = L_851_arg0
		_G["AutoSaw"] = L_852_[1]
	end)
spawn(function()
	while wait(.2) do
		pcall(function()
			if _G["AutoSaw"] then
				local L_853_ = {}
				L_853_[2] = GetConnectionEnemies("المنشار (The Saw)")
				if L_853_[2] then
					repeat
						task["wait"]()
...
Q = Tabs["Quests"]:CreateToggle("Auto Saber Sword", function(L_854_arg0)
		local L_855_ = {}
		L_855_[1] = L_854_arg0
		...
		pcall(function()
			if _G["AutoSaber"] and (L_1_[45]["Data"]["Level"]["Value"] >= 200 and (not L_1_[45]["Backpack"]:FindFirstChild("Saber") and not L_1_[45]["Character"]:FindFirstChild("Saber"))) then
				if workspace["Map"]["Jungle"]["Final"]["Part"]["Transparency"] == 0 then
						...
						if workspace["Map"]["Desert"]["Burn"]["Part"]["Transparency"] == 0 then
							...
							if L_1_[18]["Remotes"]["CommF_"]:InvokeServer("ProQuestProgress", "SickMan") ~= 0 then
								...
								elseif L_1_[18]["Remotes"]["CommF_"]:InvokeServer("ProQuestProgress", "RichSon") == 0 then
									if workspace["Enemies"]:FindFirstChild("زعيم الغوغاء (Mob Leader)") or L_1_[18]:FindFirstChild("زعيم الغوغاء (Mob Leader)") then
										...
										for L_856_forvar0, L_857_forvar1 in pairs(workspace["Enemies"]:GetChildren()) do
											...
											if L_858_[3]["Name"] == "زعيم الغوغاء (Mob Leader)" and L_1_[4]["Alive"](L_858_[3]) then
												repeat
													task["wait"]()
					...
					if workspace["Enemies"]:FindFirstChild("خبير السيف (Saber Expert)") or L_1_[18]:FindFirstChild("خبير السيف (Saber Expert)") then
						for L_859_forvar0, L_860_forvar1 in pairs(workspace["Enemies"]:GetChildren()) do
							local L_861_ = {}
							L_861_[2], L_861_[1] = L_859_forvar0, L_860_forvar1
							if L_861_[1]["Name"] == "خبير السيف (Saber Expert)" and L_1_[4]["Alive"](L_861_[1]) then
								repeat
									task["wait"]()
...
Q = Tabs["Quests"]:CreateToggle("Auto Cybrog", function(L_862_arg0)
		local L_863_ = {}
		L_863_[1] = L_862_arg0
...
spawn(function()
	while wait(.2) do
		if _G["AutoColShad"] then
			pcall(function()
				local L_864_ = {}
				L_864_[2] = GetConnectionEnemies("سايبورغ (Cyborg)")
				if L_864_[2] then
					repeat
						task["wait"]()
...
Q = Tabs["Quests"]:CreateToggle("Auto Usoap's Hat", function(L_865_arg0)
		local L_866_ = {}
		L_866_[2] = L_865_arg0
...
Q = Tabs["Quests"]:CreateToggle("Auto Bisento V2", function(L_870_arg0)
		local L_871_ = {}
		L_871_[1] = L_870_arg0
		_G["اللحية الرمادية (Greybeard)"] = L_871_[1]
	end)
spawn(function()
	while wait(Sec) do
		if _G["اللحية الرمادية (Greybeard)"] then
			pcall(function()
				...
				elseif GetWP("Bisento") then
					...
					L_872_[1] = GetConnectionEnemies("اللحية الرمادية (Greybeard)")
					if L_872_[1] then
						repeat
							wait()
							L_1_[4]["Kill"](L_872_[1], _G["اللحية الرمادية (Greybeard)"])
						until _G["اللحية الرمادية (Greybeard)"] == false or not L_872_[1]["Parent"] or L_872_[1]["Humanoid"]["Health"] <= 0
					else
...
Q = Tabs["Quests"]:CreateToggle("Auto Warden Sword", function(L_873_arg0)
		local L_874_ = {}
		L_874_[1] = L_873_arg0
...
spawn(function()
	while wait(.1) do
		if _G["WardenBoss"] then
			pcall(function()
				local L_875_ = {}
				L_875_[2] = GetConnectionEnemies("رئيس السجانين (Chief Warden)")
				if L_875_[2] then
					repeat
						wait()
...
Q = Tabs["Quests"]:CreateToggle("Auto Marine Coat", function(L_876_arg0)
		local L_877_ = {}
		L_877_[1] = L_876_arg0
...
spawn(function()
	while wait(.1) do
		if _G["MarinesCoat"] then
			pcall(function()
				local L_878_ = {}
				L_878_[2] = GetConnectionEnemies("نائب الأدميرال (Vice Admiral)")
				if L_878_[2] then
					repeat
						wait()
...
Q = Tabs["Quests"]:CreateToggle("Auto Swan Coat", function(L_879_arg0)
		local L_880_ = {}
		L_880_[1] = L_879_arg0
...
spawn(function()
	while wait(.1) do
		if _G["SwanCoat"] then
			pcall(function()
				local L_881_ = {}
				L_881_[2] = GetConnectionEnemies("سوان (Swan)")
				if L_881_[2] then
					repeat
						wait()
...
Q = Tabs["Quests"]:CreateToggle("Auto Rengoku Sword", function(L_882_arg0)
		local L_883_ = {}
		L_883_[2] = L_882_arg0
	...
	pcall(function()
		while wait(.1) do
			if _G["IceBossRen"] then
				local L_884_ = {}
				L_884_[1] = GetConnectionEnemies("أدميرال الجليد المستيقظ (Ice Admiral)")
				if L_884_[1] then
					repeat
						task["wait"]()
...
Q = Tabs["Quests"]:CreateToggle("Auto Key Rengoku", function(L_885_arg0)
		local L_886_ = {}
		L_886_[1] = L_885_arg0
		_G["KeysRen"] = L_886_[1]
	end)
...
Q = Tabs["Quests"]:CreateToggle("Auto Dragon Trident", function(L_888_arg0)
		local L_889_ = {}
		L_889_[2] = L_888_arg0
	...
	while wait(.1) do
		pcall(function()
			if _G["AutoTridentW2"] then
				local L_890_ = {}
				L_890_[2] = GetConnectionEnemies("حارس المد (Tide Keeper)")
				if L_890_[2] then
					repeat
						task["wait"]()
...
Q = Tabs["Quests"]:CreateToggle("Auto Long Sword", function(L_891_arg0)
		local L_892_ = {}
		L_892_[1] = L_891_arg0
	...
	while wait(.1) do
		pcall(function()
			if _G["LongsWord"] then
				local L_893_ = {}
				L_893_[2] = GetConnectionEnemies("ماس (Diamond)")
				if L_893_[2] then
					repeat
						task["wait"]()
...
Q = Tabs["Quests"]:CreateToggle("Auto Black Spikey", function(L_894_arg0)
		local L_895_ = {}
		L_895_[1] = L_894_arg0
...
spawn(function()
	while wait(.1) do
		if _G["BlackSpikey"] then
			pcall(function()
				local L_896_ = {}
				L_896_[1] = GetConnectionEnemies("جيريمي (Jeremy)")
				if L_896_[1] then
					repeat
						wait()
...
Q = Tabs["Quests"]:CreateToggle("Auto Dark Blade V3", function(L_897_arg0)
		local L_898_ = {}
		L_898_[2] = L_897_arg0
		...
		pcall(function()
			if _G["DarkBladev3"] and World2 then
				...
				if GetBP("Fist of Darkness") > 1 then
					if not workspace["Enemies"]:FindFirstChild("اللحية السوداء (Darkbeard)") then
						_tp(CFrame["new"](3677.08203125, 62.751937866211, -3144.8332519531))
					elseif GetConnectionEnemies("اللحية السوداء (Darkbeard)") and GetBP("Fist of Darkness") >= 1 then
						repeat
							wait()
...
Q = Tabs["Quests"]:CreateToggle("Auto Midnight Blade", function(L_899_arg0)
		local L_900_ = {}
		L_900_[1] = L_899_arg0
	...
	while wait(Sec) do
		pcall(function()
			if _G["AutoEcBoss"] then
				if GetM("إكتوبلازم (Ectoplasm)") >= 99 then
					L_1_[18]["Remotes"]["CommF_"]:InvokeServer("إكتوبلازم (Ectoplasm)", "شراء (Buy)", 3)
				elseif GetM("إكتوبلازم (Ectoplasm)") <= 99 then
					local L_901_ = {}
					L_901_[1] = GetConnectionEnemies("القبطان الملعون (Cursed Captain)")
					if L_901_[1] then
						repeat
							wait()
...
Q = Tabs["Quests"]:CreateToggle("Auto Darkbeard", function(L_902_arg0)
		local L_903_ = {}
		L_903_[1] = L_902_arg0
...
spawn(function()
	while wait(.1) do
		if _G["Auto_Def_DarkCoat"] then
			pcall(function()
				if GetBP("Fist of Darkness") and not workspace["Enemies"]:FindFirstChild("اللحية السوداء (Darkbeard)") then
					_tp(CFrame["new"](3677.08203125, 62.751937866211, -3144.8332519531))
				elseif GetConnectionEnemies("اللحية السوداء (Darkbeard)") then
					local L_904_ = {}
					L_904_[1] = GetConnectionEnemies("اللحية السوداء (Darkbeard)")
					if L_904_[1] then
						repeat
							wait()
				...
				elseif not GetBP("Fist of Darkness") and not GetConnectionEnemies("اللحية السوداء (Darkbeard)") then
					repeat
						wait(.1)
						_G["AutoFarmChest"] = true
					until not _G["Auto_Def_DarkCoat"] or GetBP("Fist of Darkness") or GetConnectionEnemies("اللحية السوداء (Darkbeard)")
					_G["AutoFarmChest"] = false
				end
			end)
		end
			...
			pcall(function()
				if (L_1_[18]["Remotes"]["CommF_"]:InvokeServer("GetUnlockables"))["FlamingoAccess"] == nil and L_1_[45]["Data"]["Level"]["Value"] >= 1500 then
					...
					for L_910_forvar0, L_911_forvar1 in pairs(L_1_[18]["Remotes"]["CommF_"]:InvokeServer("getInventoryFruits")) do
						...
						L_1_[18]["Remotes"]["CommF_"]:InvokeServer("Cousin", "شراء (Buy)")
...
Q = Tabs["Quests"]:CreateToggle("Auto Swan Glasses", function(L_922_arg0)
		local L_923_ = {}
		L_923_[1] = L_922_arg0
...
spawn(function()
	while wait(.2) do
		if _G["Auto_SwanGG"] then
			pcall(function()
				local L_924_ = {}
				L_924_[2] = GetConnectionEnemies("دون سوان (Don Swan)")
				if L_924_[2] then
					repeat
						wait()
...
Q = Tabs["Quests"]:CreateToggle("Auto Bigmom", function(L_925_arg0)
		local L_926_ = {}
		L_926_[1] = L_925_arg0
...
spawn(function()
	while wait(Sec) do
		if _G["AutoBigmom"] then
			pcall(function()
				local L_927_ = {}
				L_927_[2] = GetConnectionEnemies("ملكة الكعك (Cake Queen)")
				if L_927_[2] then
					repeat
						task["wait"]()
	...
	while wait(Sec) do
		pcall(function()
			if _G["Auto_Cavender"] then
				local L_930_ = {}
				L_930_[1] = GetConnectionEnemies("القرصان الجميل (Beautiful Pirate)")
				if L_930_[1] then
					repeat
						wait()
...
Q = Tabs["Quests"]:CreateToggle("Auto Twin Hooks", function(L_931_arg0)
		local L_932_ = {}
		L_932_[2] = L_931_arg0
...
spawn(function()
	while wait(Sec) do
		pcall(function()
			if _G["TwinHook"] then
				local L_933_ = {}
				L_933_[1] = GetConnectionEnemies("القبطان الفيل (Captain Elephant)")
				if L_933_[1] then
					repeat
						wait()
...
Q = Tabs["Quests"]:CreateToggle("Auto Serpent Bow", function(L_934_arg0)
		local L_935_ = {}
		L_935_[1] = L_934_arg0
...
spawn(function()
	while wait(Sec) do
		if _G["AutoSerpentBow"] then
			local L_936_ = {}
			L_936_[1] = GetConnectionEnemies("زعيم الهيدرا (Hydra Leader)")
			if L_936_[1] then
				repeat
					wait()
...
Q = Tabs["Quests"]:CreateToggle("Auto Lei Accessory", function(L_937_arg0)
		local L_938_ = {}
		L_938_[2] = L_937_arg0
...
spawn(function()
	while wait(.2) do
		if _G["AutoKilo"] then
			pcall(function()
				local L_939_ = {}
				L_939_[1] = GetConnectionEnemies("أدميرال الكيلو (Kilo Admiral)")
				if L_939_[1] then
					repeat
						task["wait"]()
...
Tabs["Quests"]:CreateButton("Buy Buso Colors", function()
		L_1_[18]["Remotes"]["CommF_"]:InvokeServer("ColorsDealer", "2")
	end)
Q = Tabs["Quests"]:CreateToggle("Auto Rainbow Colors", function(L_945_arg0)
		local L_946_ = {}
		L_946_[2] = L_945_arg0
	...
	pcall(function()
		while wait(Sec) do
			if _G["Auto_Rainbow_Haki"] then
				if L_1_[45]["PlayerGui"]["Main"]["مهمة (Quest)"]["Visible"] == false then
					if _G["GetQFast"] then
						if L_1_[45]["PlayerGui"]["Main"]["مهمة (Quest)"]["Visible"] == false then
				...
				elseif L_1_[45]["PlayerGui"]["Main"]["مهمة (Quest)"]["Visible"] == true and string["find"](L_1_[45]["PlayerGui"]["Main"]["مهمة (Quest)"]["Container"]["QuestTitle"]["Title"]["Text"], "ستون (Stone)") then
					local L_947_ = {}
					L_947_[1] = GetConnectionEnemies("ستون (Stone)")
					if L_947_[1] then
						repeat
							wait()
							L_1_[4]["Kill"](L_947_[1], _G["Auto_Rainbow_Haki"])
						until _G["Auto_Rainbow_Haki"] == false or L_947_[1]["Humanoid"]["Health"] <= 0 or not L_947_[1]["Parent"] or L_1_[45]["PlayerGui"]["Main"]["مهمة (Quest)"]["Visible"] == false
					else
				...
				elseif L_1_[45]["PlayerGui"]["Main"]["مهمة (Quest)"]["Visible"] == true and string["find"](L_1_[45]["PlayerGui"]["Main"]["مهمة (Quest)"]["Container"]["QuestTitle"]["Title"]["Text"], "زعيم الهيدرا (Hydra Leader)") then
					local L_948_ = {}
					L_948_[2] = GetConnectionEnemies("زعيم الهيدرا (Hydra Leader)")
					if L_948_[2] then
						repeat
							task["wait"]()
							L_1_[4]["Kill"](L_948_[2], _G["Auto_Rainbow_Haki"])
						until _G["Auto_Rainbow_Haki"] == false or L_948_[2]["Humanoid"]["Health"] <= 0 or not L_948_[2]["Parent"] or L_1_[45]["PlayerGui"]["Main"]["مهمة (Quest)"]["Visible"] == false
					else
						local L_949_ = {}
				...
				elseif L_1_[45]["PlayerGui"]["Main"]["مهمة (Quest)"]["Visible"] == true and string["find"](L_1_[45]["PlayerGui"]["Main"]["مهمة (Quest)"]["Container"]["QuestTitle"]["Title"]["Text"], "أدميرال الكيلو (Kilo Admiral)") then
					local L_950_ = {}
					L_950_[2] = GetConnectionEnemies("أدميرال الكيلو (Kilo Admiral)")
					if L_950_[2] then
						repeat
							task["wait"]()
							L_1_[4]["Kill"](L_950_[2], _G["Auto_Rainbow_Haki"])
						until _G["Auto_Rainbow_Haki"] == false or L_950_[2]["Humanoid"]["Health"] <= 0 or not L_950_[2]["Parent"] or L_1_[45]["PlayerGui"]["Main"]["مهمة (Quest)"]["Visible"] == false
					else
						_tp(CFrame["new"](2877.61743, 423.558685, -7207.31006, -0.989591599, 0, -0.143904909, 0, 1.00000012, 0, .143904924, 0, -0.989591479))
					end
				elseif L_1_[45]["PlayerGui"]["Main"]["مهمة (Quest)"]["Visible"] == true and string["find"](L_1_[45]["PlayerGui"]["Main"]["مهمة (Quest)"]["Container"]["QuestTitle"]["Title"]["Text"], "القبطان الفيل (Captain Elephant)") then
					local L_951_ = {}
					L_951_[2] = GetConnectionEnemies("القبطان الفيل (Captain Elephant)")
					if L_951_[2] then
						repeat
							task["wait"]()
							L_1_[4]["Kill"](L_951_[2], _G["Auto_Rainbow_Haki"])
						until _G["Auto_Rainbow_Haki"] == false or L_951_[2]["Humanoid"]["Health"] <= 0 or not L_951_[2]["Parent"] or L_1_[45]["PlayerGui"]["Main"]["مهمة (Quest)"]["Visible"] == false
					else
						local L_952_ = {}
				...
				elseif L_1_[45]["PlayerGui"]["Main"]["مهمة (Quest)"]["Visible"] == true and string["find"](L_1_[45]["PlayerGui"]["Main"]["مهمة (Quest)"]["Container"]["QuestTitle"]["Title"]["Text"], "القرصان الجميل (Beautiful Pirate)") then
					local L_953_ = {}
					L_953_[2] = GetConnectionEnemies("القبطان الفيل (Captain Elephant)")
					if L_953_[2] then
						repeat
							task["wait"]()
							L_1_[4]["Kill"](L_953_[2], _G["Auto_Rainbow_Haki"])
						until _G["Auto_Rainbow_Haki"] == false or L_953_[2]["Humanoid"]["Health"] <= 0 or not L_953_[2]["Parent"] or L_1_[45]["PlayerGui"]["Main"]["مهمة (Quest)"]["Visible"] == false
					else
...
Q = Tabs["Quests"]:CreateToggle("Auto Observation V2", function(L_958_arg0)
		local L_959_ = {}
		L_959_[2] = L_958_arg0
	...
	while wait(Sec) do
		if _G["AutoKenVTWO"] then
			pcall(function()
				...
				if L_1_[45]["PlayerGui"]["Main"]["مهمة (Quest)"]["Visible"] == true and string["find"](L_1_[45]["PlayerGui"]["Main"]["مهمة (Quest)"]["Container"]["QuestTitle"]["Title"]["Text"], L_1_[2]({
					...
					if L_961_[1] then
						...
						until not _G["AutoKenVTWO"] or L_961_[1]["Humanoid"]["Health"] <= 0 or L_1_[45]["PlayerGui"]["Main"]["مهمة (Quest)"]["Visible"] == false
					else
						_tp(L_960_[2])
					end
				elseif L_1_[45]["PlayerGui"]["Main"]["مهمة (Quest)"]["Visible"] == true then
					local L_962_ = {}
					L_962_[1] = GetConnectionEnemies("القبطان الفيل (Captain Elephant)")
					if L_962_[1] then
						repeat
							wait()
							L_1_[4]["Kill"](L_962_[1], _G["AutoKenVTWO"])
						until not _G["AutoKenVTWO"] or L_962_[1]["Humanoid"]["Health"] <= 0 or L_1_[45]["PlayerGui"]["Main"]["مهمة (Quest)"]["Visible"] == false
					else
						_tp(L_960_[4])
					end
				elseif L_1_[45]["PlayerGui"]["Main"]["مهمة (Quest)"]["Visible"] == false then
				...
				if not L_1_[45]["Backpack"]:FindFirstChild("Fruit Bowl") or not L_1_[45]["Character"]:FindFirstChild("Fruit Bowl") then
					...
					if L_1_[45]["Backpack"]:FindFirstChild("Fruit Bowl") or L_1_[45]["Character"]:FindFirstChild("Fruit Bowl") then
						...
						elseif L_1_[45]["Character"]["HumanoidRootPart"]["CFrame"] == L_960_[3] then
							...
							wait(.1)
							L_1_[18]["Remotes"]["CommF_"]:InvokeServer("KenTalk2", "شراء (Buy)")
						end
					end
				end
			end)
	...
	while wait(.1) do
		pcall(function()
			if _G["Bartilo_Quest"] and Lv >= 850 then
				local L_974_ = {}
				L_974_[2] = L_1_[45]["PlayerGui"]["Main"]["مهمة (Quest)"]
				if L_1_[18]["Remotes"]["CommF_"]:InvokeServer("BartiloQuestProgress", "Bartilo") == 0 then
					_G["Level"] = false
					if L_974_[2]["Visible"] == true then
						...
						if L_975_[1] then
							...
							if L_976_[2] then
								repeat
									task["wait"]()
									if not string["find"](L_1_[45]["PlayerGui"]["Main"]["مهمة (Quest)"]["Container"]["QuestTitle"]["Title"]["Text"], "Swan Pirate") then
										L_1_[18]["Remotes"]["CommF_"]:InvokeServer("AbandonQuest")
									else
				...
				elseif L_1_[18]["Remotes"]["CommF_"]:InvokeServer("BartiloQuestProgress", "Bartilo") == 1 then
					local L_977_ = {}
					_G["Level"] = false
					L_977_[2] = GetConnectionEnemies("جيريمي (Jeremy)")
					if L_977_[2] then
						repeat
							task["wait"]()
		...
		pcall(function()
			if _G["CitizenQuest"] then
				if Lv >= 1800 and (L_1_[18]["Remotes"]["CommF_"]:InvokeServer("CitizenQuestProgress"))["KilledBandits"] == false then
					if string["find"](L_1_[45]["PlayerGui"]["Main"]["مهمة (Quest)"]["Container"]["QuestTitle"]["Title"]["Text"], "Forest Pirate") and (string["find"](L_1_[45]["PlayerGui"]["Main"]["مهمة (Quest)"]["Container"]["QuestTitle"]["Title"]["Text"], "50") and L_1_[45]["PlayerGui"]["Main"]["مهمة (Quest)"]["Visible"] == true) then
						local L_980_ = {}
						...
						if L_980_[1] then
							...
							until _G["CitizenQuest"] == false or not L_980_[1]["Parent"] or L_980_[1]["Humanoid"]["Health"] <= 0 or L_1_[45]["PlayerGui"]["Main"]["مهمة (Quest)"]["Visible"] == false
						else
				...
				elseif Lv >= 1800 and (L_1_[18]["Remotes"]["CommF_"]:InvokeServer("CitizenQuestProgress"))["KilledBoss"] == false then
					local L_981_ = {}
					L_981_[2] = GetConnectionEnemies("القبطان الفيل (Captain Elephant)")
					if L_1_[45]["PlayerGui"]["Main"]["مهمة (Quest)"]["Visible"] and (string["find"](L_1_[45]["PlayerGui"]["Main"]["مهمة (Quest)"]["Container"]["QuestTitle"]["Title"]["Text"], "القبطان الفيل (Captain Elephant)") and L_1_[45]["PlayerGui"]["Main"]["مهمة (Quest)"]["Visible"] == true) then
						if L_981_[2] then
							repeat
								task["wait"]()
								L_1_[4]["Kill"](L_981_[2], _G["CitizenQuest"])
							until _G["CitizenQuest"] == false or L_981_[2]["Humanoid"]["Health"] <= 0 or not L_981_[2]["Parent"] or L_1_[45]["PlayerGui"]["Main"]["مهمة (Quest)"]["Visible"] == false
						else
...
Q = Tabs["Quests"]:CreateToggle("Auto Training Dummy", function(L_982_arg0)
		local L_983_ = {}
		L_983_[2] = L_982_arg0
...
SuperHuman = Tabs["Quests"]:CreateToggle("Auto Superhuman", function(L_986_arg0)
		local L_987_ = {}
		L_987_[2] = L_986_arg0
	...
	while wait(Sec) do
		pcall(function()
			if _G["Auto_SuperHuman"] then
				local L_988_ = {}
				L_988_[2] = L_1_[45]["Data"]["بيلي (Beli)"]["Value"]
				L_988_[1] = L_1_[45]["Data"]["شظايا (Frag)"]["Value"]
...
DeathStep = Tabs["Quests"]:CreateToggle("Auto DeathStep", function(L_989_arg0)
		local L_990_ = {}
		L_990_[1] = L_989_arg0
						...
						if GetBP("Black Leg") or (GetBP("Black Leg"))["Level"]["Value"] >= 400 then
							if workspace["Map"]["IceCastle"]["Hall"]["LibraryDoor"]["PhoeyuDoor"]["Transparency"] == 0 then
								...
								elseif not GetBP("Library Key") then
									local L_991_ = {}
									L_991_[1] = GetConnectionEnemies("أدميرال الجليد المستيقظ (Ice Admiral)")
									if L_991_[1] then
										repeat
											wait()
...
SharkManV2 = Tabs["Quests"]:CreateToggle("Auto Sharkman Karate", function(L_992_arg0)
		local L_993_ = {}
		L_993_[2] = L_992_arg0
					...
					if not GetBP("Sharkman Karate") then
						...
						if GetBP("Fishman Karate") or (GetBP("Fishman Karate"))["Level"]["Value"] >= 400 then
							...
							elseif not GetBP("Water Key") then
								local L_994_ = {}
								L_994_[2] = GetConnectionEnemies("حارس المد (Tide Keeper)")
								if L_994_[2] then
									repeat
										wait()
...
ElectricClaw = Tabs["Quests"]:CreateToggle("Auto ElectricClaw", function(L_995_arg0)
		local L_996_ = {}
		L_996_[2] = L_995_arg0
...
DragonTalon = Tabs["Quests"]:CreateToggle("Auto DragonTalon", function(L_997_arg0)
		local L_998_ = {}
		L_998_[1] = L_997_arg0
...
Godhuman = Tabs["Quests"]:CreateToggle("Auto Godhuman", function(L_999_arg0)
		local L_1000_ = {}
		...
		pcall(function()
			if _G["Auto_God_Human"] then
				if L_1_[18]["Remotes"]["CommF_"]:InvokeServer("BuyGodhuman", true) == L_1_[2]({
					...
					if GetM("حرشف تنين (Scale)") == false or GetM("حرشف تنين (Scale)") < 10 then
						if World3 then
							Lv = 1575
							_G["Level"] = true
						else
					...
					elseif GetM("ذيل سمكة (Fish Tail)") == false or GetM("ذيل سمكة (Fish Tail)") < 20 then
						if World3 then
							Lv = 1775
							_G["Level"] = true
						else
					...
					elseif GetM("قطرة غامضة (Mystic)") == false or GetM("قطرة غامضة (Mystic)") < 10 then
						if World2 then
							Lv = 1425
							_G["Level"] = true
						else
					...
					elseif GetM("خام ماغما (Magma Ore)") == false or GetM("خام ماغما (Magma Ore)") < 20 then
						if World2 then
							Lv = 1175
							_G["Level"] = true
						else
...
SanguineArt = Tabs["Quests"]:CreateToggle("Auto SanguineArt", function(L_1001_arg0)
		local L_1002_ = {}
		...
		if _G["Snaguine"] then
			pcall(function()
				...
				if not GetBP("Sanguine Art") then
					...
					if GetM("ناب مصاص دماء (Vampire)") <= 19 then
						if World2 then
							local L_1003_ = {}
					...
					if GetM("ناب مصاص دماء (Vampire)") >= 20 and GetM("شعلة شيطانية (Wisp)") <= 19 then
						if World3 then
							local L_1004_ = {}
					...
					if GetM("ناب مصاص دماء (Vampire)") >= 20 and (GetM("شعلة شيطانية (Wisp)") >= 20 and GetM("Dark Fragment") <= 1) then
						if World2 then
							local L_1005_ = {}
							L_1005_[2] = GetConnectionEnemies("اللحية السوداء (Darkbeard)")
							if L_1005_[2] then
								repeat
									task["wait"]()
...
FullMOOn = Tabs["Race"]:AddParagraph({
	["Title"] = " FullMoon Status ";
	["Content"] = ""
})
Ismirage = Tabs["Race"]:AddParagraph({
	["Title"] = L_1_[2]({
		" Mirage Island Statu",
		"s "
	});
...
Tabs["Race"]:CreateToggle("Esp Mirage Island", function(L_1012_arg0)
		local L_1013_ = {}
...
Tabs["Race"]:CreateToggle("Auto Collect Gear", function(L_1021_arg0)
		local L_1022_ = {}
...
Tabs["Race"]:CreateButton("Talk With Stone", function()
		...
		((L_1_[18]:WaitForChild("Remotes")):WaitForChild("CommF_")):InvokeServer("RaceV4Progress", "انتقال (Teleport)");
		((L_1_[18]:WaitForChild("Remotes")):WaitForChild("CommF_")):InvokeServer("RaceV4Progress", "Continue")
	end)
Tabs["Race"]:CreateToggle("Auto Look At Moon", function(L_1041_arg0)
		local L_1042_ = {}
...
Tabs["Race"]:CreateToggle("Look Moon + Auto V3", function(L_1043_arg0)
		local L_1044_ = {}
...
RaceMink = Tabs["Race"]:CreateToggle("Auto Upgrade Mink", function(L_1046_arg0)
		local L_1047_ = {}
...
RaceHuman = Tabs["Race"]:CreateToggle("Auto Upgrade Human", function(L_1049_arg0)
		local L_1050_ = {}
...
RaceSky = Tabs["Race"]:CreateToggle("Auto Upgrade Angel", function(L_1053_arg0)
		local L_1054_ = {}
...
RaceFish = Tabs["Race"]:CreateToggle("Auto Upgrade FishMan", function(L_1059_arg0)
		local L_1060_ = {}
...
PullLv = Tabs["Race"]:CreateToggle("Auto Pull Lever", function(L_1062_arg0)
		local L_1063_ = {}
...
Train = Tabs["Race"]:CreateToggle("Auto Train V4", function(L_1067_arg0)
		local L_1068_ = {}
			...
			if _G["AcientOne"] then
				...
				for L_1070_forvar0 = 1, #L_1069_[2], 1 do
					if (L_1_[45]["Character"]:FindFirstChild("RaceEnergy"))["Value"] == 1 then
						vim1:SendKeyEvent(true, "Y", false, game)
						L_1_[18]["Remotes"]["CommF_"]:InvokeServer("UpgradeRace", "شراء (Buy)")
	...
	while wait(Sec) do
		pcall(function()
			if _G["TPDoor"] then
				...
				elseif tostring(L_1_[45]["Data"]["Race"]["Value"]) == "سايبورغ (Cyborg)" then
				...
				if tostring(L_1_[45]["Data"]["Race"]["Value"]) == "Fishman" then
					if GetSeaBeastTrial() then
						repeat
							...
							Useskills("اشتباك (Melee)", "Z")
							Useskills("اشتباك (Melee)", "X")
							Useskills("اشتباك (Melee)", "C")
							wait(.1)
							Useskills("سيف (Sword)", "Z")
							Useskills("سيف (Sword)", "X")
							wait(.1)
							Useskills("فاكهة بلوكس (Fruit)", "Z")
							Useskills("فاكهة بلوكس (Fruit)", "X")
							Useskills("فاكهة بلوكس (Fruit)", "C")
...
spawn(function()
	while wait(Sec) do
		pcall(function()
			if _G["Complete_Trials"] then
				if tostring(L_1_[45]["Data"]["Race"]["Value"]) == "سايبورغ (Cyborg)" then
...
DojoQ = Tabs["Prehistoric"]:CreateToggle("Auto Dojo Trainer", function(L_1091_arg0)
		local L_1092_ = {}
...
function printBeltName(L_1093_arg0)
	...
	if type(L_1094_[1]) == "table" and L_1094_[1]["مهمة (Quest)"]["BeltName"] then
		return L_1094_[1]["مهمة (Quest)"]["BeltName"]
	end
end
spawn(function()
	while wait(Sec) do
...
BlazeEM = Tabs["Prehistoric"]:CreateToggle("Auto Dragon Hunter", function(L_1098_arg0)
		local L_1099_ = {}
...
Toggle = Tabs["Prehistoric"]:CreateToggle("Auto Drago (V1)", function(L_1122_arg0)
		local L_1123_ = {}
...
fireflower = Tabs["Prehistoric"]:CreateToggle("Auto Drago (V2)", function(L_1124_arg0)
		local L_1125_ = {}
...
Toggle = Tabs["Prehistoric"]:CreateToggle("Auto Drago (V3)", function(L_1131_arg0)
		local L_1132_ = {}
...
Toggle = Tabs["Prehistoric"]:CreateToggle("Auto Train Drago v4", function(L_1136_arg0)
		local L_1137_ = {}
		...
		pcall(function()
			if _G["TrainDrago"] then
				...
				for L_1139_forvar0 = 1, #L_1138_[2], 1 do
					if (L_1_[45]["Character"]:FindFirstChild("RaceEnergy"))["Value"] == 1 then
						vim1:SendKeyEvent(true, "Y", false, game)
						L_1_[18]["Remotes"]["CommF_"]:InvokeServer("UpgradeRace", "شراء (Buy)", 2)
...
bdrago = Tabs["Prehistoric"]:CreateToggle("Swap Drago Race", function(L_1144_arg0)
		local L_1145_ = {}
...
Tabs["Prehistoric"]:CreateButton("Craft Dragonheart", function()
		local L_1151_ = {}
		L_1151_[1] = {
			[1] = "CraftItem",
			[2] = "Craft",
...
Tabs["Prehistoric"]:CreateButton("Craft Dragonstorm", function()
		local L_1152_ = {}
		L_1152_[1] = {
			[1] = "CraftItem",
			[2] = "Craft",
...
Tabs["Prehistoric"]:CreateButton("Craft Dino Hood", function()
		local L_1153_ = {}
		L_1153_[2] = {
			[1] = "CraftItem",
			[2] = "Craft";
...
Tabs["Prehistoric"]:CreateButton("Craft T-Rex Skull", function()
		local L_1154_ = {}
		L_1154_[2] = {
			[1] = "CraftItem",
			[2] = "Craft";
...
Kaura = Tabs["Prehistoric"]:CreateToggle("Kill Aura", function(L_1187_arg0)
		local L_1188_ = {}
...
Tabs["SeaEvent"]:CreateToggle("Ship Speed Modifier", function(L_1207_arg0)
		local L_1208_ = {}
...
Tabs["SeaEvent"]:CreateSlider("Ship Speed", 0, 1000, 300, function(L_1213_arg0)
		local L_1214_ = {}
		L_1214_[1] = L_1213_arg0
		SetSpeedBoat = L_1214_[1]
	end)
Tabs["SeaEvent"]:CreateToggle("Auto Press W", function(L_1215_arg0)
		local L_1216_ = {}
...
Tabs["SeaEvent"]:CreateToggle("No Clip Ship", function(L_1218_arg0)
		local L_1219_ = {}
...
Tabs["SeaEvent"]:CreateButton("Craft SharkTooth", function()
		local L_1226_ = {}
		L_1226_[1] = {
			[1] = "CraftItem";
			[2] = "Craft",
...
Tabs["SeaEvent"]:CreateButton("Craft TerrorJaw", function()
		local L_1227_ = {}
		L_1227_[2] = {
			[1] = "CraftItem";
			[2] = "Craft",
...
Tabs["SeaEvent"]:CreateButton("Craft SharkAnchor", function()
		local L_1228_ = {}
		L_1228_[2] = {
			[1] = "CraftItem";
			[2] = "Craft";
...
Tabs["SeaEvent"]:CreateButton("Craft LeviathanCrown", function()
		local L_1229_ = {}
		L_1229_[1] = {
			[1] = "CraftItem";
			[2] = "Craft";
...
Tabs["SeaEvent"]:CreateButton("Craft LeviathanBoat", function()
		local L_1231_ = {}
		L_1231_[1] = {
			[1] = "CraftItem";
			[2] = "Craft",
...
Tabs["SeaEvent"]:CreateButton("Craft MythicalScroll", function()
		local L_1233_ = {}
		L_1233_[1] = {
			[1] = "CraftItem",
			[2] = "Craft",
...
Tabs["SeaEvent"]:CreateButton("Buy Boats", function()
		L_1_[18]["Remotes"]["CommF_"]:InvokeServer("BuyBoat", _G["SelectedBoat"])
	end)
Q = Tabs["SeaEvent"]:CreateDropdown({
...
Q = Tabs["SeaEvent"]:CreateToggle("Auto Sail Boat", function(L_1238_arg0)
		local L_1239_ = {}
...
Tabs["SeaEvent"]:CreateToggle("Auto Shark", function(L_1249_arg0)
		local L_1250_ = {}
		L_1250_[1] = L_1249_arg0
		_G["Shark"] = L_1250_[1]
	end)
Tabs["SeaEvent"]:CreateToggle("Auto Piranha", function(L_1251_arg0)
		local L_1252_ = {}
		L_1252_[2] = L_1251_arg0
		_G["Piranha"] = L_1252_[2]
	end)
Tabs["SeaEvent"]:CreateToggle("Auto Terror Shark", function(L_1253_arg0)
		local L_1254_ = {}
...
Tabs["SeaEvent"]:CreateButton("Trade Items Azure", function()
		(L_1_[18]["Modules"]["Net"]:FindFirstChild("RF/KitsuneStatuePray")):InvokeServer()
	end)
Tabs["SeaEvent"]:CreateButton({
	["Name"] = L_1_[2]({
...
Tabs["SeaEvent"]:CreateButton("Buy Spy", function()
	...
	for L_1335_forvar0, L_1336_forvar1 in next, workspace:GetChildren() do
		...
		pcall(function()
			if DevilFruitESP then
				if string["find"](L_1337_[3]["Name"], "فاكهة (Fruit)") then
...
Tabs["Esp"]:CreateToggle("Esp Berry", function(L_1386_arg0)
		local L_1387_ = {}
...
Tabs["Esp"]:CreateToggle("Esp Player", function(L_1391_arg0)
		local L_1392_ = {}
...
Tabs["Esp"]:CreateToggle("Esp Chest", function(L_1396_arg0)
		local L_1397_ = {}
...
Tabs["Esp"]:CreateToggle("Esp Fruit", function(L_1401_arg0)
		local L_1402_ = {}
...
Tabs["Esp"]:CreateToggle("Esp Island", function(L_1406_arg0)
		local L_1407_ = {}
...
Tabs["Esp"]:CreateToggle("Esp Flower", function(L_1411_arg0)
		local L_1412_ = {}
...
Tabs["Esp"]:CreateToggle("Esp Legendary Sword", function(L_1416_arg0)
		local L_1417_ = {}
...
Tabs["Esp"]:CreateToggle("Esp Haki Color", function(L_1418_arg0)
		local L_1419_ = {}
...
Tabs["Esp"]:CreateToggle("Esp Gear", function(L_1420_arg0)
		local L_1421_ = {}
		L_1421_[2] = L_1420_arg0
		ESPGear = L_1421_[2]
		if not L_1421_[2] then
			for L_1422_forvar0, L_1423_forvar1 in pairs(workspace["Map"]["MysticIsland"]:GetDescend)
Tabs["Esp"]:CreateToggle("Esp SeaEvent Island", function(L_1425_arg0)
		local L_1426_ = {}
...
Tabs["Esp"]:CreateToggle("Esp Advanced Dealer", function(L_1430_arg0)
		local L_1431_ = {}
...
stockParagraph = Tabs["Raids"]:AddParagraph({
	["Title"] = "Stock Fruit";
	["Content"] = "Loading..."
})
task["spawn"](function()
...
RandomFF = Tabs["Raids"]:CreateToggle("Auto Random Fruit", function(L_1444_arg0)
		local L_1445_ = {}
	...
	while wait(Sec) do
		pcall(function()
			if _G["Random_Auto"] then
				L_1_[55]["Remotes"]["CommF_"]:InvokeServer("Cousin", "شراء (Buy)")
			end
		end)
	end
end)
DropF = Tabs["Raids"]:CreateToggle("Auto Drop Fruit", function(L_1446_arg0)
		local L_1447_ = {}
...
StoredF = Tabs["Raids"]:CreateToggle("Auto Store Fruit", function(L_1448_arg0)
		local L_1449_ = {}
...
TwF = Tabs["Raids"]:CreateToggle("Auto Tween to Fruit", function(L_1450_arg0)
		local L_1451_ = {}
		...
		if _G["TwFruits"] then
			pcall(function()
				for L_1452_forvar0, L_1453_forvar1 in pairs(workspace:GetChildren()) do
					...
					if string["find"](L_1454_[3]["Name"], "فاكهة (Fruit)") then
...
BringF = Tabs["Raids"]:CreateToggle("Auto Collect Fruit", function(L_1455_arg0)
		local L_1456_ = {}
...
Tabs["Raids"]:CreateDropdown("Select Fruit Shop", {
		"Rocket-Rocket",
		"Spin-Spin";
		"Blade-Blade",
		"Spring-Spring";
		...
		"Kitsune-Kitsune";
		"Dragon-Dragon"
	}, function(L_1457_arg0)
		local L_1458_ = {}
		L_1458_[1] = L_1457_arg0;
		(getgenv())["SelectFruit"] = L_1458_[1]
	end)
Tabs["Raids"]:CreateToggle("Auto Buy Fruit Shop", function(L_1459_arg0)
		local L_1460_ = {}
...
AutoChipBeli = Tabs["Raids"]:CreateToggle("Auto Buy Chip [Beli]", function(L_1480_arg0)
		local L_1481_ = {}
...
StartR = Tabs["Raids"]:CreateToggle("Auto Start Raid", function(L_1488_arg0)
		local L_1489_ = {}
...
Tabs["Raids"]:CreateToggle("Auto Awakening", function(L_1507_arg0)
		local L_1508_ = {}
...
Tabs["Raids"]:CreateToggle("Auto Teleport To Lab", function(L_1509_arg0)
		local L_1510_ = {}
...
Tabs["Raids"]:CreateButton("Buy Microchip Law", function()
		L_1_[55]["Remotes"]["CommF_"]:InvokeServer("BlackbeardReward", "Microchip", "2")
	end)
Tabs["Raids"]:CreateButton("Start Law Raids", function()
...
Tabs["Raids"]:CreateToggle("Auto Start Law Raids", function(L_1513_arg0)
		local L_1514_ = {}
...
Tabs["Raids"]:CreateToggle("Auto Kill Law", function(L_1515_arg0)
		local L_1516_ = {}
...
spawn(function()
	while wait(Sec) do
		if _G["AutoLawKak"] then
			pcall(function()
				local L_1517_ = {}
				L_1517_[1] = GetConnectionEnemies("أوردر (Order)")
				if L_1517_[1] then
					repeat
						task["wait"]()
...
Tabs["Raids"]:CreateToggle("Auto Farm Dungeon", function(L_1519_arg0)
		local L_1520_ = {}
...
Tabs["Raids"]:CreateToggle("TP Exit (1)", function(L_1526_arg0)
		local L_1527_ = {}
...
L_1_[8] = function()
	...
	for L_1529_forvar0, L_1530_forvar1 in pairs(workspace["Map"]["سرداب (Dungeon)"]:GetChildren()) do
		local L_1531_ = {}
...
Tabs["Raids"]:CreateToggle("TP Exit (2)", function(L_1533_arg0)
		local L_1534_ = {}
...
task["spawn"](function()
	while task["wait"](.3) do
		...
		for L_1536_forvar0, L_1537_forvar1 in pairs(workspace["Map"]["سرداب (Dungeon)"]:GetChildren()) do
			local L_1538_ = {}
...
Tabs["Raids"]:CreateToggle("TP Exit (3)", function(L_1539_arg0)
		local L_1540_ = {}
...
L_1_[66] = function()
	local L_1541_ = {}
	for L_1542_forvar0, L_1543_forvar1 in pairs(workspace["Map"]["سرداب (Dungeon)"]:GetChildren()) do
		local L_1544_ = {}
...
Tabs["Raids"]:CreateToggle("TP Exit (4)", function(L_1546_arg0)
		local L_1547_ = {}
...
L_1_[128] = function()
	...
	for L_1549_forvar0, L_1550_forvar1 in pairs(workspace["Map"]["سرداب (Dungeon)"]:GetChildren()) do
		local L_1551_ = {}
...
Tabs["Combat"]:CreateDropdown("Select Aim Method", {
		"Aim Player",
		"Nearest Aim"
	}, function(L_1566_arg0)
		local L_1567_ = {}
		L_1567_[2] = L_1566_arg0
		ABmethod = L_1567_[2]
	end)
Tabs["Combat"]:CreateToggle("Aimbot Method Skills", function(L_1568_arg0)
		local L_1569_ = {}
...
Tabs["Combat"]:CreateButton("Get player quests", function()
		pcall(function()
...
spawn(function()
	while task["wait"]() do
		if _G["AutoPlayerHunter"] then
			if game["Players"]["LocalPlayer"]["PlayerGui"]["Main"]["مهمة (Quest)"]["Visible"] == false then
				task["wait"](.5);
				(game:GetService("ReplicatedStorage"))["Remotes"]["CommF_"]:InvokeServer("PlayerHunter")
			else
				for L_1588_forvar0, L_1589_forvar1 in pairs((game:GetService("Workspace"))["Characters"]:GetChildren()) do
					...
					if string["find"](game["Players"]["LocalPlayer"]["PlayerGui"]["Main"]["مهمة (Quest)"]["Container"]["QuestTitle"]["Title"]["Text"], L_1590_[3]["Name"]) then
						repeat
							task["wait"]()
							if AutoHaki then
								AutoHaki()
...
Tabs["Combat"]:CreateToggle("Auto Enable PvP", function(L_1591_arg0)
		local L_1592_ = {}
...
Tabs["Combat"]:CreateToggle("Auto Safe Mode", function(L_1594_arg0)
		local L_1595_ = {}
...
Tabs["Combat"]:CreateToggle("Enable Fly", function(L_1618_arg0)
		local L_1619_ = {}
		L_1619_[2] = L_1618_arg0
		L_1_[102](L_1619_[2])
	end)
Tabs["Combat"]:CreateSlider("Speed Fly Mode", 10, 200, 50, function(L_1620_arg0)
		local L_1621_ = {}
		L_1621_[2] = L_1620_arg0
		L_1_[91](L_1621_[2])
	end)
Tabs["Combat"]:CreateToggle("Dash No Cooldown", function(L_1622_arg0)
		local L_1623_ = {}
...
Tabs["Combat"]:CreateToggle("Ignore Same Teams", function(L_1641_arg0)
		local L_1642_ = {}
		L_1642_[2] = L_1641_arg0
		_G["NoAimTeam"] = L_1642_[2]
	end)
Tabs["Combat"]:CreateToggle("Accept Allies", function(L_1643_arg0)
		local L_1644_ = {}
...
Tabs["Travel"]:CreateButton("Travel Zou (World 3)", function()
...
Travelllll = Tabs["Travel"]:CreateDropdown({
	...
	["Callback"] = function(L_1651_arg0)
		...
		_G["جزيرة (Island)"] = L_1652_[2]
	end
})
GoIsland = Tabs["Travel"]:CreateToggle("Auto Travel", function(L_1653_arg0)
		local L_1654_ = {}
		L_1654_[1] = L_1653_arg0
		_G["انتقال (Teleport)"] = L_1654_[1]
		if L_1654_[1] then
			for L_1655_forvar0, L_1656_forvar1 in pairs(workspace["_WorldOrigin"]["Locations"]:GetChildren()) do
				...
				if L_1657_[3]["Name"] == _G["جزيرة (Island)"] then
					repeat
						wait()
						_tp(L_1657_[3]["CFrame"] * CFrame["new"](0, 30, 0))
					until not _G["انتقال (Teleport)"] or Root["CFrame"] == L_1657_[3]["CFrame"]
				end)
Tabs["Travel"]:AddSection({
	"Travel - Portal"
})
...
Tabs["Travel"]:CreateButton("requestEntrance", function()
...
GoNPCs = Tabs["Travel"]:CreateToggle("Auto Tween to NPC", function(L_1665_arg0)
		local L_1666_ = {}
...
Tabs["المتجر (Shop)"]:AddSection({
	"Shop Options"
})
Tabs["المتجر (Shop)"]:CreateButton("Buy Buso", function()
		L_1_[55]["Remotes"]["CommF_"]:InvokeServer("BuyHaki", "Buso")
	end)
Tabs["المتجر (Shop)"]:CreateButton("Buy Geppo", function()
		L_1_[55]["Remotes"]["CommF_"]:InvokeServer("BuyHaki", "Geppo")
	end)
Tabs["المتجر (Shop)"]:CreateButton("Buy Soru", function()
		L_1_[55]["Remotes"]["CommF_"]:InvokeServer("BuyHaki", "Soru")
	end)
Tabs["المتجر (Shop)"]:CreateButton("Buy Ken", function()
		L_1_[55]["Remotes"]["CommF_"]:InvokeServer("KenTalk", "شراء (Buy)")
	end)
Tabs["المتجر (Shop)"]:AddSection({
	"Fighting - Style"
})
Tabs["المتجر (Shop)"]:CreateButton("Buy Black Leg", function()
		L_1_[55]["Remotes"]["CommF_"]:InvokeServer("BuyBlackLeg")
	end)
Tabs["المتجر (Shop)"]:CreateButton("Buy Electro", function()
		L_1_[55]["Remotes"]["CommF_"]:InvokeServer("BuyElectro")
	end)
Tabs["المتجر (Shop)"]:CreateButton("Buy Fishman Karate", function()
		L_1_[55]["Remotes"]["CommF_"]:InvokeServer("BuyFishmanKarate")
	end)
Tabs["المتجر (Shop)"]:CreateButton("Buy DragonClaw", function()
		L_1_[55]["Remotes"]["CommF_"]:InvokeServer("BlackbeardReward", "DragonClaw", "2")
	end)
Tabs["المتجر (Shop)"]:CreateButton("Buy Superhuman", function()
		L_1_[55]["Remotes"]["CommF_"]:InvokeServer("BuySuperhuman")
	end)
Tabs["المتجر (Shop)"]:CreateButton("Buy Death Step", function()
		L_1_[55]["Remotes"]["CommF_"]:InvokeServer("BuyDeathStep")
	end)
Tabs["المتجر (Shop)"]:CreateButton("Buy Sharkman Karate", function()
		L_1_[55]["Remotes"]["CommF_"]:InvokeServer("BuySharkmanKarate")
	end)
Tabs["المتجر (Shop)"]:CreateButton("Buy ElectricClaw", function()
		L_1_[55]["Remotes"]["CommF_"]:InvokeServer("BuyElectricClaw")
	end)
Tabs["المتجر (Shop)"]:CreateButton("Buy DragonTalon", function()
		L_1_[55]["Remotes"]["CommF_"]:InvokeServer("BuyDragonTalon")
	end)
Tabs["المتجر (Shop)"]:CreateButton("Buy Godhuman", function()
		L_1_[55]["Remotes"]["CommF_"]:InvokeServer("BuyGodhuman")
	end)
Tabs["المتجر (Shop)"]:CreateButton("Buy SanguineArt", function()
		L_1_[55]["Remotes"]["CommF_"]:InvokeServer("BuySanguineArt")
	end)
Tabs["المتجر (Shop)"]:AddSection({
	"Accessory"
})
Tabs["المتجر (Shop)"]:CreateButton("Buy Tomoe Ring", function()
		L_1_[55]["Remotes"]["CommF_"]:InvokeServer("BuyItem", "Tomoe Ring")
	end)
Tabs["المتجر (Shop)"]:CreateButton("Buy Black Cape", function()
		L_1_[55]["Remotes"]["CommF_"]:InvokeServer("BuyItem", "Black Cape")
	end)
Tabs["المتجر (Shop)"]:CreateButton("Buy Swordsman Hat", function()
		L_1_[55]["Remotes"]["CommF_"]:InvokeServer("BuyItem", "Swordsman Hat")
	end)
Tabs["المتجر (Shop)"]:CreateButton("Buy Bizarre Rifle", function()
		L_1_[55]["Remotes"]["CommF_"]:InvokeServer("إكتوبلازم (Ectoplasm)", "شراء (Buy)", 1)
	end)
Tabs["المتجر (Shop)"]:CreateButton("Buy Ghoul Mask", function()
		L_1_[55]["Remotes"]["CommF_"]:InvokeServer("إكتوبلازم (Ectoplasm)", "شراء (Buy)", 2)
	end)
Tabs["المتجر (Shop)"]:AddSection({
	"Weapon World1"
})
Tabs["المتجر (Shop)"]:CreateButton("Buy Cutlass", function()
		L_1_[55]["Remotes"]["CommF_"]:InvokeServer("BuyItem", "Cutlass")
	end)
Tabs["المتجر (Shop)"]:CreateButton("Buy Katana", function()
		L_1_[55]["Remotes"]["CommF_"]:InvokeServer("BuyItem", "Katana")
	end)
Tabs["المتجر (Shop)"]:CreateButton("Buy Iron Mace", function()
		L_1_[55]["Remotes"]["CommF_"]:InvokeServer("BuyItem", "Iron Mace")
	end)
Tabs["المتجر (Shop)"]:CreateButton("Buy Duel Katana", function()
		L_1_[55]["Remotes"]["CommF_"]:InvokeServer("BuyItem", "Duel Katana")
	end)
Tabs["المتجر (Shop)"]:CreateButton("Buy Triple Katana", function()
		L_1_[55]["Remotes"]["CommF_"]:InvokeServer("BuyItem", "Triple Katana")
	end)
Tabs["المتجر (Shop)"]:CreateButton("Buy Pipe", function()
		L_1_[55]["Remotes"]["CommF_"]:InvokeServer("BuyItem", "Pipe")
	end)
Tabs["المتجر (Shop)"]:CreateButton({
	["Name"] = L_1_[2]({
...
Tabs["المتجر (Shop)"]:CreateButton("Buy Bisento", function()
		L_1_[55]["Remotes"]["CommF_"]:InvokeServer("BuyItem", "Bisento")
	end)
Tabs["المتجر (Shop)"]:CreateButton("Buy Soul Cane", function()
		L_1_[55]["Remotes"]["CommF_"]:InvokeServer("BuyItem", "Soul Cane")
	end)
Tabs["المتجر (Shop)"]:CreateButton("Buy Slingshot", function()
		L_1_[55]["Remotes"]["CommF_"]:InvokeServer("BuyItem", "Slingshot")
	end)
Tabs["المتجر (Shop)"]:CreateButton("Buy Musket", function()
		L_1_[55]["Remotes"]["CommF_"]:InvokeServer("BuyItem", "Musket")
	end)
Tabs["المتجر (Shop)"]:CreateButton("Buy Dual Flintlock", function()
		L_1_[55]["Remotes"]["CommF_"]:InvokeServer("BuyItem", "Dual Flintlock")
	end)
Tabs["المتجر (Shop)"]:CreateButton("Buy Flintlock", function()
...
Tabs["المتجر (Shop)"]:CreateButton("Buy Cannon", function()
		L_1_[55]["Remotes"]["CommF_"]:InvokeServer("BuyItem", "Cannon")
	end)
Tabs["المتجر (Shop)"]:CreateButton("Buy Kabucha", function()
		L_1_[55]["Remotes"]["CommF_"]:InvokeServer("BlackbeardReward", "Slingshot", "2")
	end)
Tabs["المتجر (Shop)"]:AddSection({
	"Fragments shop"
})
Tabs["المتجر (Shop)"]:CreateButton("Buy Refund Stats", function()
		L_1_[55]["Remotes"]["CommF_"]:InvokeServer("BlackbeardReward", "Refund", "2")
	end)
Tabs["المتجر (Shop)"]:CreateButton("Buy Reroll Race", function()
		L_1_[55]["Remotes"]["CommF_"]:InvokeServer("BlackbeardReward", "Reroll", "2")
	end)
Tabs["المتجر (Shop)"]:CreateButton("Buy Ghoul Race", function()
		L_1_[55]["Remotes"]["CommF_"]:InvokeServer("إكتوبلازم (Ectoplasm)", " Change", 4)
	end)
Tabs["المتجر (Shop)"]:CreateButton({
	["Name"] = L_1_[2]({
		"Buy Cyborg Race (2.5";
...
Tabs["متنوع (Misc)"]:AddSection({
	"Server - Function"
})
Tabs["متنوع (Misc)"]:CreateButton({
...
Tabs["متنوع (Misc)"]:CreateButton("Rejoin Server", function()
		(game:GetService("TeleportService")):Teleport(game["PlaceId"], game["Players"]["LocalPlayer"])
	end)
Tabs["متنوع (Misc)"]:CreateButton({
	["Name"] = "Hop Server";
...
Tabs["متنوع (Misc)"]:CreateButton("Teleport [Job ID]", function()
...
Tabs["متنوع (Misc)"]:CreateButton("Copy JobID", function()
...
Tabs["متنوع (Misc)"]:CreateButton("Open Title Selection", function()
		L_1_[103]["Remotes"]["CommF_"]:InvokeServer("getTitles", true)
		L_1_[42]["PlayerGui"]["Main"]["Titles"]["Visible"] = true
	end)
DisbleChat = Tabs["متنوع (Misc)"]:CreateToggle("Disable Chat GUI", function(L_1694_arg0)
		local L_1695_ = {}
...
DisbleLeaderB = Tabs["متنوع (Misc)"]:CreateToggle({
	["Name"] = L_1_[2]({
...
Tabs["متنوع (Misc)"]:CreateButton("Set Pirate Team", function()
		Pirates()
	end)
Tabs["متنوع (Misc)"]:CreateButton("Set Marine Team", function()
		Marines()
	end)
UnPortal = Tabs["متنوع (Misc)"]:CreateToggle("Unlock All Portals", function(L_1702_arg0)
		local L_1703_ = {}
...
HakiStat = Tabs["متنوع (Misc)"]:CreateDropdown({
...
Tabs["متنوع (Misc)"]:CreateButton("ChangeBusoStage", function()
...
rtxM = Tabs["متنوع (Misc)"]:CreateToggle({
...
Tabs["متنوع (Misc)"]:CreateButton("Turn on Fast Mode", function()
		for L_1709_forvar0, L_1710_forvar1 in next, workspace:GetDescend)
Tabs["متنوع (Misc)"]:CreateButton("Turn on Low CPU", function()
		LowCpu()
	end)
Tabs["متنوع (Misc)"]:CreateButton({
	["Name"] = L_1_[2]({
...
Tabs["متنوع (Misc)"]:CreateButton("Remove Sky Fog", function()
...
Tabs["متنوع (Misc)"]:CreateButton({
	...
	["Callback"] = function()
		for L_1715_forvar0, L_1716_forvar1 in pairs((game:GetObjects(L_1_[2]({
			...
			if L_1717_[3]["فاكهة (Fruit)"]:FindFirstChild("AnimationController") then
				((L_1717_[3]["فاكهة (Fruit)"]:FindFirstChild("AnimationController")):LoadAnimation(L_1717_[3]["فاكهة (Fruit)"]:FindFirstChild("Idle"))):Play()
			end
...
briggt1 = Tabs["متنوع (Misc)"]:CreateToggle("Turn on Full Bright", function(L_1720_arg0)
		local L_1721_ = {}
...
DayN = Tabs["متنوع (Misc)"]:CreateDropdown("Select Time", {
		"Day",
		"Night"
	}, function(L_1722_arg0)
		local L_1723_ = {}
		L_1723_[2] = L_1722_arg0
		_G["SelectDN"] = L_1723_[2]
	end)
dayornight = Tabs["متنوع (Misc)"]:CreateToggle("Turn on Time", function(L_1724_arg0)
		local L_1725_ = {}
...
walkWater = Tabs["متنوع (Misc)"]:CreateToggle({
	["Name"] = L_1_[2]({
...
iceWalk = Tabs["متنوع (Misc)"]:CreateToggle("Turn on Ice Walk", function(L_1728_arg0)
		local L_1729_ = {}
		...
		if _G["WalkWater"] then
			pcall(function()
				if L_1_[42]["Character"] and L_1_[42]["Character"]:FindFirstChild("LeftFoot") then
					...
					L_1730_[2]["مكتمل (Done)"]:Connect(function()
						L_1730_[3]:Destroy()
					end)
					L_1730_[2]:Play()
				end
...
task["spawn"](function()
	RunSer["Heartbeat"]:Connect(function()
		pcall(function()
			...
			if L_1767_[5] == "فاكهة بلوكس (Fruit)" then
				if L_1767_[1] then
					local L_1768_ = {}
