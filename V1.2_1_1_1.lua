...
function Library:CreateWindow(titleText)
    ...
    local CloseBtn = Create("ImageButton", {Size = UDim2.fromOffset(20, 20), Position = UDim2.new(1, -30, 0, 8), BackgroundTransparency = 1, Image = "rbxassetid://9886659671", ImageColor3 = Color3.fromRGB(200, 200, 200), Parent = TopBar})
    ...
    local Sidebar = Create("ScrollingFrame", {
        ...
        CanvasSize = UDim2.new(0, 0, 0, 0), -- Auto-calculated
    ...
    local Window = {Sidebar = Sidebar, Container = Container}
    local firstTab = true
    ...
    function Window:CreateTab(name)
        ...
        -- Compatibility Layer for WindUI/Rayfield style calls
        function Tab:Section(data)
            local SecFrame = Create("Frame", {
                ...
                Create("TextLabel", {
                    ...
                    BackgroundTransparency = 1, 
                    Parent = nil -- Will be parented by Frame
                })
            })
            SecFrame:FindFirstChildOfClass("TextLabel").Parent = SecFrame
            return SecFrame
        end

        function Tab:Paragraph(data)
            local obj = self:CreateButton(data.Title .. ": " .. (data.Desc or ""), nil)
            function obj:SetDesc(desc)
                pcall(function()
                    obj.Text = data.Title .. ": " .. tostring(desc)
                end)
            end
            function obj:SetTitle(title)
                pcall(function()
                    data.Title = title
                    obj.Text = tostring(title) .. ": " .. (data.Desc or "")
                end)
            end
            return obj
        end
        ...
        function Tab:Toggle(data)
            return self:CreateToggle(data.Title, data.Callback)
        end

        function Tab:Dropdown(data)
            return self:CreateDropdown(data.Title, data.Values or data.Options or {}, data.Callback)
        end

        function Tab:Slider(data)
            local SliderFrame = Create("Frame", {Size = UDim2.new(0.96, 0, 0, 45), BackgroundColor3 = Color3.fromRGB(18, 18, 18), Parent = Page}, {Create("UICorner", {CornerRadius = UDim.new(0, 6)})})
            ApplyPremiumBorder(SliderFrame, 1)
            local Title = Create("TextLabel", {Text = data.Title, Font = Enum.Font.GothamMedium, TextSize = 11, TextColor3 = Color3.fromRGB(200, 200, 200), TextXAlignment = "Left", BackgroundTransparency = 1, Position = UDim2.fromOffset(10, 5), Size = UDim2.new(1, -60, 0, 15), Parent = SliderFrame})
            local ValueLabel = Create("TextLabel", {Text = tostring(data.Default or data.Min or 0), Font = Enum.Font.GothamBold, TextSize = 11, TextColor3 = Color3.fromRGB(255, 255, 255), TextXAlignment = "Right", BackgroundTransparency = 1, Position = UDim2.new(1, -50, 0, 5), Size = UDim2.new(0, 40, 0, 15), Parent = SliderFrame})
            local SliderBar = Create("Frame", {Size = UDim2.new(1, -20, 0, 4), Position = UDim2.fromOffset(10, 30), BackgroundColor3 = Color3.fromRGB(35, 35, 35), Parent = SliderFrame}, {Create("UICorner", {CornerRadius = UDim.new(1, 0)})})
            local Fill = Create("Frame", {Size = UDim2.fromScale(((data.Default or 0) - data.Min) / (data.Max - data.Min), 1), BackgroundColor3 = Color3.fromRGB(220, 220, 220), Parent = SliderBar}, {Create("UICorner", {CornerRadius = UDim.new(1, 0)})})
            ...
            return {Set = function(_, v) local pos = math.clamp((v - data.Min) / (data.Max - data.Min), 0, 1); ValueLabel.Text = tostring(v); Fill.Size = UDim2.fromScale(pos, 1); if data.Callback then data.Callback(v) end end}
        end

        function Tab:Button(data)
            return self:CreateButton(data.Title, data.Callback)
        end

        function Tab:Input(data)
            local InputFrame = Create("Frame", {Size = UDim2.new(0.96, 0, 0, 35), BackgroundColor3 = Color3.fromRGB(18, 18, 18), Parent = Page}, {Create("UICorner", {CornerRadius = UDim.new(0, 6)})})
            ApplyPremiumBorder(InputFrame, 1)
            Create("TextLabel", {Text = data.Title, Font = Enum.Font.GothamMedium, TextSize = 11, TextColor3 = Color3.fromRGB(200, 200, 200), TextXAlignment = "Left", BackgroundTransparency = 1, Position = UDim2.fromOffset(10, 0), Size = UDim2.new(0.4, 0, 1, 0), Parent = InputFrame})
            local TextBox = Create("TextBox", {Size = UDim2.new(0.5, 0, 0, 25), Position = UDim2.new(1, -10, 0.5, 0), AnchorPoint = Vector2.new(1, 0.5), BackgroundColor3 = Color3.fromRGB(25, 25, 25), Text = "", PlaceholderText = "Type here...", Font = Enum.Font.Gotham, TextSize = 10, TextColor3 = Color3.fromRGB(255, 255, 255), Parent = InputFrame}, {Create("UICorner", {CornerRadius = UDim.new(0, 4)})})
            TextBox.FocusLost:Connect(function(enter) if enter and data.Callback then data.Callback(TextBox.Text) end end)
            return {Set = function(_, v) TextBox.Text = tostring(v); if data.Callback then data.Callback(v) end end}
        end
        ...
        function Tab:CreateDropdown(text, options, callback)
            local DropFrame = Create("Frame", {Size = UDim2.new(0.96, 0, 0, 35), BackgroundColor3 = Color3.fromRGB(18, 18, 18), Parent = Page, ClipsDescendants = true}, {Create("UICorner", {CornerRadius = UDim.new(0, 6)})})
            ApplyPremiumBorder(DropFrame, 1)
            
            local DropTitle = Create("TextLabel", {Text = text .. " : " .. (options[1] or "None"), Font = Enum.Font.GothamMedium, TextSize = 11, TextColor3 = Color3.fromRGB(200, 200, 200), TextXAlignment = "Left", BackgroundTransparency = 1, Position = UDim2.fromOffset(10, 0), Size = UDim2.new(1, -40, 0, 35), Parent = DropFrame})
            ...
            DropBtn.MouseButton1Click:Connect(function()
                opened = not opened
                local targetHeight = opened and (35 + math.min(#options * 30, 150)) or 35
                TweenService:Create(DropFrame, TweenInfo.new(0.3), {Size = UDim2.new(0.96, 0, 0, targetHeight)}):Play()
            end)
            
            local function AddOption(opt)
                local OptBtn = Create("TextButton", {Size = UDim2.new(1, 0, 0, 28), BackgroundColor3 = Color3.fromRGB(25, 25, 25), Text = opt, TextColor3 = Color3.fromRGB(180, 180, 180), Font = Enum.Font.Gotham, TextSize = 10, Parent = OptionsFrame}, {Create("UICorner", {CornerRadius = UDim.new(0, 4)})})
            ...
            function DropObj:Add(val)
                AddOption(val)
                local count = 0
                for _, v in pairs(OptionsFrame:GetChildren()) do if v:IsA("TextButton") then count = count + 1 end end
                OptionsFrame.CanvasSize = UDim2.new(0, 0, 0, count * 30)
                OptionsFrame.Size = UDim2.new(1, 0, 0, math.min(count * 30, 150))
            end
            return DropObj
        end
...
-- Initialize UI Early to prevent errors
    Win = Library:CreateWindow("Vylera Hub | Blox Fruit")
    
    -- Function to ensure first tab is visible
    local function SelectFirstTab()
        local first = true
        for _, v in pairs(Win.Sidebar:GetChildren()) do
            if v:IsA("TextButton") and first then
                -- Trigger click logic
                for _, btn in pairs(Win.Sidebar:GetChildren()) do 
                    if btn:IsA("TextButton") then 
                        TweenService:Create(btn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(20, 20, 20), TextColor3 = Color3.fromRGB(200, 200, 200)}):Play() 
                    end 
                end
                for _, page in pairs(Win.Container:GetChildren()) do 
                    if page:IsA("ScrollingFrame") then page.Visible = false end
                end
                TweenService:Create(v, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(220, 220, 220), TextColor3 = Color3.fromRGB(20, 20, 20)}):Play()
                local pageName = v.Text
                -- This is a bit hacky but works since we know the structure
                for _, page in pairs(Win.Container:GetChildren()) do
                    -- The first page created will be the first one in Container
                    page.Visible = true
                    break
                end
                first = false
    ...
    Tabs.InfoTab = CreateTab("Information")
    Tabs.MainTab = CreateTab("Farming")
Tabs.OthersTab = CreateTab("Others")
Tabs.ItemsTab = CreateTab("Items")
Tabs.SettingsTab = CreateTab("Settings")
Tabs.LocalPlayerTab = CreateTab("Player")
Tabs.StatsTab = CreateTab("Stats")
Tabs.SeaEventTab = CreateTab("Sea Event")
Tabs.SeaStackTab = CreateTab("Sea Stack")
Tabs.SeaSettingsTab = CreateTab("Sea Settings")
Tabs.DragonDojoTab = CreateTab("Dragon Dojo")
Tabs.RaceTab = CreateTab("Race V4")
Tabs.CombatTab = CreateTab("Combat")
Tabs.RaidTab = CreateTab("Raid")
Tabs.EspTab = CreateTab("Esp")
Tabs.TeleportTab = CreateTab("Teleport")
Tabs.ShopTab = CreateTab("Shop")
Tabs.FruitTab = CreateTab("Fruit")
Tabs.MiscTab = CreateTab("Misc")
	    Tabs.ServerTab = CreateTab("Server")
	
	-- Ensure first tab is active
	task.defer(SelectFirstTab)
...
-- UI already initialized at top


-- UI Elements (Simplified Port)
-- Note: Premium V11 is more compact, so we only port essential toggles/buttons
-- For a full port, we would need to manually map thousands of lines.
-- I will include the core farming toggles.


Tabs.MainTab:Dropdown({Title = "Select Weapon", Values = {"Melee", "Sword", "Blox Fruit", "Gun"}, Callback = function(v) _G.Settings.Main["Select Weapon"] = v end})
Tabs.MainTab:Dropdown({Title = "Farm Method", Values = {"Quest", "Chest", "Monster"}, Callback = function(v) _G.Settings.Main["Farm Level Method"] = v end})
Tabs.MainTab:Toggle({Title = "Auto Farm Level", Callback = function(v) _G.Settings.Main["Auto Farm"] = v end})
Tabs.MainTab:Toggle({Title = "Auto Fast Farm", Callback = function(v) _G.Settings.Main["Auto Fast Farm"] = v end})

Tabs.SettingsTab:Dropdown({Title = "Fast Attack Mode", Values = {"Normal", "Fast", "Slow"}, Callback = function(v) _G.Settings.Setting["Fast Attack Mode"] = v end})
Tabs.SettingsTab:Toggle({Title = "Fast Attack", Callback = function(v) _G.Settings.Setting["Fast Attack"] = v end})
