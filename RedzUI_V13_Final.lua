...
local function ApplyV13Style(parent, thickness)
    if not parent then return end
    if parent:FindFirstChild("V13Stroke") then return end
    local stroke = Instance.new("UIStroke")
    stroke.Name = "V13Stroke"
    stroke.Thickness = thickness or 2.8
    stroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
    stroke.Parent = parent
    local gradient = Instance.new("UIGradient")
    gradient.Name = "V13Gradient"
...
task.spawn(function()
    while task.wait(0.5) do
        local gui = game:GetService("CoreGui"):FindFirstChild("redz library") or game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild("redz library")
        if gui then
            for _, v in pairs(gui:GetDescendants()) do
                if (v:IsA("Frame") or v:IsA("TextButton") or v:IsA("ScrollingFrame")) and v.Size.Y.Offset > 15 then
                    ApplyV13Style(v, 2.2)
                    if v.Name == "Main" or v.Name == "MainFrame" then
                        ApplyV13Style(v, 3.5)
